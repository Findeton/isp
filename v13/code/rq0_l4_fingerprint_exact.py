...
DECLARED and gated for corridor membership BEFORE any fixture value is
computed.  The corridor gates run on an OFF-FIXTURE population -- the 48
records of the committed carrier that are not one of the four committed
boundaries -- and a FREEZE BARRIER gate certifies, from recorded per-gate
provenance flags, that no corridor gate touched the fixture.  Only after the
barrier do the fixture, amnesty and census stages run.  The receipt's gate
order is the proof.
...
_TOUCHED_FIXTURE = False            # set by every evaluation of a fixture
                                    # boundary; read by the barrier gate
...
def gate(gid: str, cls: str, claim: str, ok: bool, value=None,
    ...
    """A gate records a claim, its verdict, and whether the fixture had been
    touched when it fired.  `must_pass` marks DERIVATION gates: a false one
    exits 1.  Corridor and fixture findings are substantive and exit 0."""
    GATES.append({"id": gid, "class": cls, "claim": claim, "passed": bool(ok),
                  "must_pass": bool(must_pass), "value": value,
                  "fixture_touched_when_fired": _TOUCHED_FIXTURE})
    return bool(ok)
...
def act_part(part, sigma):
    ...
    return tuple(sorted(tuple(sorted(sigma[j] for j in r)) for r in part))
...
def declare_candidates():
    ...
    prog("candidate family: declared and frozen (6 candidates)")
    reg = []
    ...
    gate("L4-00", "freeze",
         ...
         "and C3 and three worker constructions.",
...
def run_corridor():
    prog("corridor gates: nondegeneracy, monotonicity, factoring, "
         "covariance, identity-free side -- OFF-FIXTURE ONLY")
    OFF = off_fixture()
...
def freeze_barrier():
    """THE BARRIER.  Certified from the recorded per-gate provenance flags:
    no gate fired so far had touched a fixture boundary."""
    ok = (not _TOUCHED_FIXTURE) and all(
        not g["fixture_touched_when_fired"] for g in GATES)
    if MUTANT == "freeze-lax":
        touch(PTOMO)
        ok = not _TOUCHED_FIXTURE
    gate("L4-FREEZE", "freeze",
         "FREEZE BARRIER.  Every gate above is a candidate declaration, a "
         "corridor gate or a derivation cross-check, and NONE of them "
         "evaluated ANY declared candidate at ANY of the four committed "
         "boundaries: the corridor population is the 48 off-fixture records "
         "and the 745 identity-free census patches.  (The anchors above "
         "reproduce terminal values, which are immutable base and not this "
         "unit's fixture truth.)  Every gate below computes fixture truth. "
         "RUNBOOK sec 13(4).",
         ok, {"gates_before_the_barrier": len(GATES)}, must_pass=True)
...
def run_covariance():
    ...
    gate("L4-COV4", "covariance",
         ...
         "candidate is both covariant and non-resolution at DET: the "
         "intersection is empty, exactly as L4-COV3 proves it must be.",
...
def verdict():
    ...
    tags = []
    if survivors:
        tags.append("RQ0-L4-NOMOLOGICAL-FINGERPRINT (on the pin's letter; "
                    "qualified -- see the dichotomy)")
    if amnesties:
        tags.append("RQ0-L4-FINGERPRINT-AMNESTY")
    tags.append("RQ0-L4-CLASS-IMPOSSIBILITY (over the declared class at the "
                "committed configuration, and over the declared family under "
                "the inherited corridor)")
    ...
    gate("L4-DICH", "verdict",
         "THE FINGERPRINT DICHOTOMY, over the declared family: every "
         "separation this unit found is EITHER entailed by the refinement "
         "order -- hence a grading of coarseness, which survives the amnesty "
         "sweep for the same state-free reason the terminal inversion "
         "theorem holds -- OR not so entailed, in which case it reads the "
         "declared state and the amnesty sweep kills it.  No separation is "
         "both.",
...
def build_receipt():
    return {
        ...
        "arithmetic": "exact rationals throughout; no float in any "
                      "substantive path",
        "gate_order_proves_the_freeze": True,
...
MUTANTS = ["anchor:A03", "anchor:A04", "anchor:A08", "anchor:A10",
           "anchor:A11", "anchor:A13", "anchor:A15",
