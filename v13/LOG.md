# v13 — THE DOUBLE WELD (GRAVITY): append-only ledger

House discipline in force (pins before receipts; exact arithmetic
where instruments permit; anchors exit-1-only; green-unreviewed not
citable; hostile rounds confer terminal; forward-only corrections;
kill conditions fire without appeal).

## 2026-07-28 — FOUNDED FROM THE DRAFT; GW1 PINNED AND DISPATCHED
## (v13 LEDGER #1)
paper 0 (relativistic-isp-v13-paper0-gravity.md) stands as
**[DRAFT]** — a programme prospectus, committed at 34d46ef with
provenance declared (an external synthesis note, typeset and
depersonalized; content verbatim).  The draft is NOT upgraded by
this entry: no claim in it is adjudicated, and its grades remain
the cited papers' own.  What this entry does: freezes v13's first
unit, **note-gw1-metric-from-closure-pin.md** — the draft's §6
GW1 operationalized, with STEP 0 (the instrument census: does any
committed substrate carry BOTH the order-metric instrument and
metric-free deformation kernels?) promoted to a first-class
possible verdict (GW1-BLOCKED-AT-⟨instrument⟩), the draft's kill
condition verbatim (insertion or underdetermination ⇒ the algebra
represents rather than explains), all five controls, and the
three-defect separation (Δᴮ ≠ Ω ≠ Riemann) engraved in scope.
User-authorized 2026-07-28 ("pin the v13 draft and run GW1")
alongside v12's W2-W5 batch.  Dispatched in parallel with them.

## 2026-07-28 — GW1 DELIVERED: **GW1-BLOCKED-AT-KERNEL** — THE
## INSTRUMENT CENSUS IS THE VERDICT, GREEN-UNREVIEWED (v13 LEDGER #2)
Step 0 decided the unit (note-gw1-metric-from-closure.md, 419
lines; no gw1 code written, per the pin — the census IS the
deliverable; adjudicator spot-verification: the two load-bearing
committed receipts re-run — v6_task2f's Carrollian no-go and
v6_p2d's insertion exhibit — both exactly as censused).
**INSTRUMENT (i), q^ij_order: PRESENT AND RUNNABLE** — Minkowski
recovered from interval cardinalities (g^xx = +0.99; 3D spatial
block ≈ −I₂; boost-covariant); the CURVED coefficient 1/Ω²
recovered from two independent causal channels with Ω never
inserted (chains +0.993, counts +1.000, channels agree +0.995);
plus the oriented frame, v8's conformal+volume receipts, v10's
ruler card.  **INSTRUMENT (ii), metric-free lapse-parameterized
J_a[N] with a nontrivial two-cell: ABSENT — four committed
families, each dropping exactly one requirement: (1)**
record-native lapse-parameterized kernels are metric-free but the
two-cell is EXACTLY ZERO (the pointwise-additive no-go, re-run:
|comm| = 0 at N = 2000/4000/8000; and the down-set advance is
irreversible — J⁻¹ does not exist, the pinned two-cell cannot be
formed); **(2)** the grid embedding-flow has a genuine two-cell
but the metric is INSERTED at the unit-normal normalization — the
insertion exhibited by a COMMITTED control (with Ω in the normal:
corr = +1.0000; Ω removed, same target: +0.0109 — the two-cell
returns what was put in; had step 0 been forced through here,
GW1-INSERTED fires with this exhibit); **(3)** the Γ-level lattice
two-cell is real and exact but h^ij enters via ½{γ^i,γ^j} and v2
Prop 10.6 is an all-order SIGN-AMBIGUITY no-go (a GW1-style
underdetermination already committed); **(4)** the record-history
holonomies (d42b3/d49/d72/d74; v11 ld/u2/u3) are metric-free,
exact, and NONTRIVIAL — but have NO LAPSE: no N, no ε, no ∂_j —
the closure relation has no referents.  **The one construction
with all three properties — v3 paper 2's Candidate B,
E_a^mix[N,M;Δ] → K_∥[N∂_xM − M∂_xN] — HAS NO CODE ANYWHERE**
(v3/ contains zero .py) and is 1+1D.  **THE PROGRAMME
CONSEQUENCE, scoped: v13's decisive missing object is a
metric-free, lapse-parameterized deformation kernel with a
nontrivial exchange two-cell on a record substrate — GW1's
successor is either the Candidate-B implementation (1+1D, the
honest first rung) or a lapse structure grafted onto the
record-history holonomies (family 4, which has everything but the
lapse).**  Worker-disclosed drift: v6_p2c PART 2 re-runs at
...
## 2026-07-30 — RQ0-L0 DOUBLE HOSTILE REJECTION ADJUDICATED:
## POSITIVE RUNGS WITHDRAWN; FRESH L0 REPAIR CYCLE AUTHORIZED (v13 LEDGER #30)
The two frozen reports at #28 and #29 have been checked directly against the
immutable pin, estimator, fixtures, scorer, note and receipts.  Both `REJECT`
verdicts are adopted.  The live claims `RQ0-LOCALIZATION-GROUPOID` and
`RQ0-LOCAL-ATLAS` are withdrawn.  Commit #26 remains immutable historical
evidence and its self-description remains **GREEN-UNREVIEWED**; it is not a
citable positive L0 result.  No old file or ledger entry is rewritten.

**F1 / MISSING `Comp_D` ACCEPTED.**  The pin types `Comp_D` as a flat table of
implemented composites and makes mixed joint implementation and faithful
composition mandatory for an independently addressable complement.  Direct
source inspection confirms that `OperationalDataset` has no such field and
that the factor search tests only generated star-algebra dimension,
commutation and scalar intersection.  The exact `D+`/`D-` two-world argument
therefore holds: worlds differing only in admitted mixed composites serialize
identically and cannot be separated by the frozen estimator.  The delivery
establishes an algebra factorization, not operational addressability.

**F2 / GENERATOR-PRESENTATION OBSTRUCTION ACCEPTED.**  The adjudicator
reproduced both exact mutants against the frozen estimator.  The public
calibration `(X1,Z1,X2,Z2)` returns ambient dimension 16 with one valid and one
finest factorization.  Replacing `X2` by `X1 X2` generates the same exact
star-algebra, contains `X2` in that algebra, but returns zero valid and zero
finest factorizations.  Merely adding `X1 X2` as a redundant accessible
composite also preserves ambient dimension 16 and returns zero
factorizations.  The estimator depends on a specially factor-aligned primitive
generator surface.  The cancelling-circuit control sends the identical matrix
to the estimator and does not answer this objection.

**F3 / C10 ACCEPTED.**  The stored JSON receipt contains 14 counted local
objects, ten record-bearing objects and four recordless singleton atoms.  This
contradicts the pin's mandatory requirement that every positive local object
counted toward the main atlas carry a nonempty independently derived record
algebra.  Filtering after construction does not preserve the advertised
meet-category claim without a new object and overlap definition.

**F4 / AMBIGUITY ACCEPTED.**  Exact rerun of the ambiguity fixture returns one
finest factorization and `is_factorization_ambiguous = false`.  With the
supplied presentation swap it returns atom maps `(0,1)` and `(1,0)`; removing
that supplied swap while leaving the operational data unchanged returns only
`(0,1)`.  This transports a declared automorphism of one factorization; it does
not recover several operationally indistinguishable factorizations.  The
current `RQ0-LOCALIZATION-GROUPOID` gate would in fact reject a genuinely
multiple-finest main result because its hidden-truth map requires exactly one.

**F5 / RESTRICTIONS AND DESCENT ACCEPTED.**  The algebraic joins, meets,
intersection dimensions, W3 witness rows and record-to-algebra membership are
substantive and retained.  `LocalRestriction` and `RecordRestriction`, however,
are constructed in the scorer from truth-mapped atom sets and record handles.
The path gate checks tuple-key presence; it constructs no exact algebra
homomorphism, projector pullback, direct/composite naturality square or
distinct `FactIface` object.  The exact common-witness membership survives;
typed L0 record descent does not.

**F6 / CONTROL AND RECEIPT DEFECTS ACCEPTED.**  Direct mutants confirm that
`dataset_is_exact_conjugate` accepts changed `support_actions` and changed
`presentation_actions`.  The static no-smuggling detector catches the chosen
fixture import but not a hard-coded atom partition or dataset-handle branch.
The equal-law control computes equal fair marginals but supplies
`bridge_exists = false` and its obstruction literally; `4 != 8` excludes the
declared fixed-carrier isomorphism only.  The outcome logic sets
`receipt_valid` from anchors, caps and classification, so an invalid state with
no scientific outcome can exit zero.  Mandatory composition, C10, genuine
ambiguity and typed-descent gates are absent.  Thus `56/56` certifies only the
implemented rows.

**NON-STAR WORDING ACCEPTED.**  The exact 14/50/66/134 lattice counts remain.
Once four atoms are selected, however, the complete proper-join Boolean
lattice and its non-star meet nerve are generated by construction.  They are
not an independently learned irregular regional incidence structure.

The following exact results survive as calibration data and lemmas: exact
`Q(zeta_8)` arithmetic; estimator-before-fixture chronology; reachable-support
removal; eight operational signatures at the supplied access scope;
exhaustive 4,140 generator partitions; the selected-surface four-factor
star-algebra with atom dimensions `(4,4,4,4)` and ambient dimension 256; its
proper-join lattice and exact intersections; six W3 write/preserve/erase/no-
write rows; record attachment by exact matrix membership; the 576-cell phase
discriminator; finite `mu_8` scope; deterministic receipt regeneration; and
the observed-anchor failure.  No causal, geometric, field or gravity claim was
smuggled.

**CURRENT LIVE OUTCOME:** no positive L0 rung is secure.  The explicit
candidate class and the exact `D+`/`D-` plus generator-presentation
obstructions justify the scientific diagnosis
`RQ0-L0-BLOCKED-AT-ADDRESS`.  It is not conferred terminal status in this
entry because the negative must be gated natively in a fail-closed receipt.

The user's current go-ahead and the pre-existing pin's anti-tailoring clause
authorize one forward-only L0 repair cycle.  Because the estimator schema must
change after fixture truth was opened, the old estimator may not be edited and
the old fixture may not be rescored to restore a positive headline.  The next
event must be a new strict addressability-repair pin.  It must pre-register a
new estimator path and a genuinely unseen, preferably non-Pauli-tensor fixture
family; typed `Comp_D`; `D+`/`D-`; generator-change and redundant-composite
invariance; faithful joint composition; C10-compliant record-bearing objects
and overlap closure; genuine multiple-factorization ambiguity; executable
algebra/record maps and triple naturality; computed same-law no-bridge search;
end-to-end gauge/phase checks; hard-coded-truth and handle-branch mutants; and
fully fail-closed outcomes.  The new estimator must be committed and frozen
before any new fixture truth exists.
...
## 2026-08-06 — CYCLE B′ R1 (OPERATOR LENS) REVIEW FROZEN; PANEL
## COMPLETE (v13 LEDGER #131)
review-rq0-composite-operator.md frozen; compliance verified (SHA
re-checks twice, no cross-reading, 29-row numbers table).  R1
VERDICT: ACCEPT-WITH-FIXES; NO false computed number in H1 (65/65
sweep, 30/30 lemma pairs, 10/10 hostile composites, all 8
gap-table rows incl. every Schmidt-rank multiset, reproduced by a
deliberately different route).  **MAJOR (K2 lands): the corrected
entanglement witness has a DEMONSTRATED FALSE NEGATIVE at rank
profile (2,2)** — an admitted PVM at the committed dimension
({|00⟩⟨00|+|Ψ⁺⟩⟨Ψ⁺|, |11⟩⟨11|+|Ψ⁻⟩⟨Ψ⁻|}) with provably ENTANGLED
rank-two atoms (partial-transpose det = −1/16) opens the gap while
the unit's flag defaults False (only rank-one atoms were
classified); the PARITY row's decisive word "separable" was
ASSERTED, not computed (true — R1 measured it by exact PPT and by
the orthogonal product decomposition — but ungated).  R1 BOUNDED
the defect exactly: at 2⊗2 only (2,2) can produce a false negative
(proofs for the other profiles supplied); repair ≈ 15 lines of
exact PPT.  MODERATES: Thm 3.2's proof one-sided (refinement
direction silently uses unitality) and its "exactly" clause false
without the block-indicator-admitted hypothesis THE UNIT CHECKS
BUT THE PAPER OMITS (3-dim unital counterexample supplied); "the
joint class GENERATED by the local ones" names a closure the code
does not compute — the unit uses the LINEAR SPAN of simple tensors
(what Lemma 3.1 requires); under the ∗-algebra reading
HOSTILE⊗C2 goes dim 6→16 and its core 2→4 — the hostile wing's
trivial core exists ONLY under the span reading (must be said);
§6.4's diagnosis contradicted by §7.6 (THIRD independent
convergence on the rotated-not-manufactured correction).  K1
POSITIVE: Lemma 3.1 re-proved sound; R1 went past the protocol and
DIRECTLY TESTED six of the 16 excluded carrier-25 pairs (incl.
manufactured×manufactured and C5×C5) — all distribute exactly; the
carrier-20 cap was more conservative than needed.  PANEL TOTALS:
3× ACCEPT-WITH-FIXES; ~79 independently recomputed quantities
across three lenses; ZERO false computed numbers; every casualty
...
## 2026-08-06 — THE STAGE-4 FORK RESOLVED BY THE USER'S WORD;
## CYCLE B″ PINNED: THE ATLAS AXIOM (v13 LEDGER #135)
The user: "go ahead" on the synthesis plan — branches A and B
FUSED (the binding mechanism is the patch's own generating
dynamics, committed structure in every cycle, hence NO NEW
PRIMITIVE; branch C stays the measured fallback).
note-rq0-l2-generative-admissibility-pin.md frozen (base
efa7224): the candidate ATLAS AXIOM — admissible iff (i)
Galois-minimality both ways (#103 minimality + the terminal
Cycle B closure) AND (ii) GENERATIVITY (the declared family is
generated by the patch's own realized dynamics; realized-legs /
reachable-subprocess / counter-law instruments); the four
mandatory discriminators (the colluding pair MUST FAIL with the
killing condition named per member; the eraser passes; the
relabelled and counter-law contexts honest; the arena's dependent
pair reported); THE FEYNMAN GATE (does admissibility ever change
a tester statistic — EMPIRICALLY-IDLE first-class); THE MINIMAL
WITNESS (≤ 5 configurations, printed whole); THE COLLUSION-COST
CONJECTURE (forging cost = minimal law alteration, exact; growth
along the tower; the Borges bound: total forgery = another
world — with CHEAP-LAW-FORGERY and BLOCKED-AT-⟨cost-referent⟩ as
first-class negatives).  Pre-registered outcomes:
...
## 2026-08-09 — RSQ VERIFIED BYTE-IDENTICAL; PROTOCOL FROZEN; PANEL
## DISPATCHED (v13 LEDGER #305)
Adjudicator verification PASSED (inline selftest exit 0;
artifacts regenerated byte-identically).
note-rsq-hostile-protocol.md FROZEN (K1 the permutation-module
obstruction + its relation to the LCB wall / K2 the
grown-arena FOUND control's meaning / K3 the precheck
architecture + the stillborn-cell meeting / K4 HA-at-d=3 +
