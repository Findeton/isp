# POT — the potential: a loop family built, a discriminator gated, and a ladder that cannot see the area

**Status:** `DELIVERED` — built against the frozen pin `v14/note-pot-pin.md`
(sha256-12 `df2f15efa7b0`). Verified to run: two plain runs byte-identical,
every gate passed, every declared mutant dead at its declared target, the
falsification self-test fatal at every anchor class and writing nothing.
Between delivery and adjudication every headline below is a **candidate
reading**.

## The Loop Observable on This Carrier Is a Function of the Perimeter Alone at Every Configuration, So the Area Leg of the Discriminator Is Family-Invariant While Its Winding Leg Partitions the Inventory

**Unit:** POT, v14, paper #36.
**Instrument:** `v14/code/pot_exact.py`.
**Artifacts:** `v14/code/pot_output.txt`, `v14/code/pot_receipt.json`.
**Inheritance, hash-verified at run time and by no other route:** the allowed
weight systems, the coupling inventory, the extreme points, the odd twist and
the merging-index law come from ACT, `v14/paper-34-act.md` (`d933221780ed`),
with its instrument `v14/code/act_exact.py` (`a90559ee0e0f`) and its receipt
`v14/code/act_receipt.json` (`7fd1267bddc7`); the conserved-price frame and
the law-native control's stamp come from SMU, `v14/paper-27-smu.md`
(`6df0db523d32`); the withholding machinery this unit inverts and the
transitivity criterion come from paper-23, `v14/paper-23-measure.md`
(`79cc67b4f6cd`); the lattice, the link operator and the holonomy definition
come from R5, `v14/paper-18-gauge-rung.md` (`62cfe5689d2c`), with its
instrument `v14/code/r5_gauge_exact.py` (`0d98de793b79`) and its receipt
`v14/code/r5_gauge_receipt.json` (`0c02b7684e5b`). Every object below is
**reimplemented** from those definitions; nothing is imported from any other
unit's program. Each digest is bound to its own path, so a pair of them
exchanged between two sources dies on the delivery run. Anchors are
(path, value) pairs and (window, consumer) pairs, not only file bytes:
9 file-bytes anchors, 29 path-value anchors and 15 verbatim-text anchors,
53 anchors in all — each verbatim window pinned by the digest of its exact
bytes, by its own frozen character count and by a declared length floor, each
located exactly once, each perturbed at a content-bearing token and required
to stop being locatable, and each bound to the gate that consumes it, where
the consumer is checked against this run's own ledger.
**Exact arithmetic only, and multiplicative by declaration:** the field is
$\mathbb{Q}(\zeta_8)$ carried as integer four-tuples over the basis
$(1,z,z^2,z^3)$ reduced modulo $z^4+1$ with a per-row power-of-two scale, so
the whole holonomy census runs in machine integers and tuple equality is
field equality once the scales agree; every weight is a strictly positive
`Fraction`; every expectation is a ratio of exact rational sums. An AST scan
of the instrument's own syntax tree is a gate, and it carries the clause the
pin's framing needs: **no call to a logarithm, an exponential, a square root
or a power exists in this source at all**, and no floating-point literal
occurs anywhere.
**The seal (#119), native from birth, total, and taken at value-close:** every
published object is digested at the moment the gate that vouches *its own
values* passes; every top-level key of the receipt is either sealed that way
or named in the declaration with the reason it cannot be; and the artifacts
are written to temporaries, read back and compared against the gate-time
digests **before** either is moved into place — the receipt by total byte
equality as well as by its seals, so the declared-unsealed keys, the gate
ledger and its chain among them, cross the disk boundary guarded.

**The verdict, quoted exactly as the instrument emits it.** Every value is
derived inside a gate from a measured receipt field; the complete string —
head included — is compared for equality against an *independent
reconstruction* that reads only the serialized receipt, derives the head by a
second head law of its own over the tallies rather than the rows, and
re-renders every segment with its own format strings; and the paper's fenced
blocks are compared as a **multiset** against the single block this run
licenses, so neither a stale verdict nor a forged twin beside the clean one
can be delivered:

```
POT-SPLIT-BY-WEIGHT-THE-WINDING-LEG-SPLITS-96-NONZERO-AND-40-ZERO-OF-136-EXTREME-POINTS-AND-1-OF-4-DECLARED-ROWS-WHILE-THE-AREA-LEG-IS-FAMILY-INVARIANT-AREA-BLIND-AT-149-OF-149-MEASURED-ROWS -- LOOP-FAMILY=192-PLACEMENTS-192-DISTINCT-LOOPS-144-CONTRACTIBLE-AND-48-WINDING;RECTANGLE-EXTENTS=1-2-3-MEASURED-SIMPLE-AT-9-OF-16-SWEPT-SIZES;ORBITS-UNDER-THE-ANCHORED-CHART-GROUP-OF-ORDER-32=11-CLOSED-TRUE;THE-WINDOW=RECTANGLE-CIRCUITS-PLUS-STRAIGHT-AND-STAIRCASE-WINDING-CYCLES-NOT-ALL-CLOSED-PATHS -- DISCRIMINATOR=WELL-DEFINED-AT-L-4-WITH-2-AREA-DISCRIMINATING-COMPARISONS-AND-4-CREUTZ-RUNGS;LEG-AREA=BLIND-EVERYWHERE;LEG-CREUTZ=DEFINED-AT-136-OF-136-EXTREME-POINTS-AND-EQUAL-TO-ONE-AT-24;LEG-WINDING=THE-ORDER-PARAMETER-ANALOGUE -- (a)CONDITIONAL=4-ROWS-ALL-STAMPED-CONDITIONAL-ON-THE-DECLARED-WEIGHTS;PLAQUETTE-EXPECTATIONS=13/10@THE-NULL,262244/65615@THE-WILSON-CONJUGATION-SYMMETRIC,4294967399/4294967375@THE-EXHIBITED-WITNESS,225/152@THE-LAW-NATIVE-CONTROL;WINDING=3-ZERO-AND-1-NONZERO;THE-CONTROL-CARRIES-LAW-RATED-CONSTRUCTION-DECLARED-AT-AN-UNPINNED-IDENTIFICATION-AND-IS-NEVER-SPENT-AS-DERIVED -- (b)FAMILY-SWEPT=136-EXTREME-POINTS-64-VERTICES-AND-72-EDGE-MIDPOINTS-ALL-RUN;FAMILY-INVARIANT=area_seen;COUPLING-DEPENDENT=ladder_word+creutz_is_unit+winding_word+mode_A+mode_B+mode_C+active_modes;SLICES=9-EXACT-ONE-PARAMETER-ROWS-AGREE-AT-THE-AREA-LEG -- PRICE=THE-INVENTORY-IS-135-COUPLINGS-AND-THE-AREA-LEG-PARTITIONS-NONE-OF-THEM;THE-WINDING-LEG-PARTITIONS-96-AGAINST-40;THE-HALVING-MODE-IS-ABSENT-AT-72-OF-136;THE-PERIMETER-MODE-IS-ABSENT-AT-40-OF-136;THE-CREUTZ-RATIO-IS-UNDEFINED-AT-0-OF-136 -- SPECTRAL-DOOR=THE-TRANSFER-MATRIX-ON-A-TIME-SLICE-IS-NAMED-NOT-RUN-AT-167772160000-STATES-PER-SLICE;THE-FINITE-FORM-IS-MEASURED=W=A+B*P+C*2^-P-WITH-SPECTRUM-{1,1,1/2}-AND-GAP-1/2-AT-640-OF-640-COINS;THE-WINDING-HOLONOMY-IS-UNITARY-AT-640-OF-640-SO-ITS-SPECTRUM-CARRIES-NO-DECAY;SPC-INHERITS-THE-GAP -- ORIENTATION=DECLARED-NEVER-DERIVED-NEITHER-STANDARD-IS-CLAIMED-TRUE-DECLARATION-RELATIVE-AT-THE-ORIENTATION-READING;THE-ORIENTED-SUBGROUP-HAS-INDEX-2-AT-BOTH-READINGS;COUPLINGS=135-AT-ANCHORED-AND-135-AT-ANCHORED-ORIENTED-AND-79-AT-THE-EXTENSION-AND-79-AT-EXTENSION-ORIENTED-SO-THE-DECLARATION-BUYS-OBSERVABLES-AND-COSTS-NO-COUPLING;THE-ODD-PART-IS-LAWFUL-ONLY-THERE-AND-IS-NON-ZERO-AT-3760-OF-11520-SHAPE-BY-COIN-TRACES-AT-9-OF-18-SHAPES-AND-AT-96-OF-136-EXTREME-POINTS -- L-BOUNDARY=THE-MERGING-INDEX-IS-8/GCD(L,8)-WHICH-IS-2-HERE-AND-1-AT-L-8;AREA-BLINDNESS-AND-THE-CLOSED-FORM-AND-THE-GAP-SURVIVE-THE-BOUNDARY-0-DISAGREEMENTS-OVER-44800-CHECKS-AND-640-OF-640-COINS-CARRY-IDENTICAL-COEFFICIENTS;THE-CLASS-MERGING-DOES-NOT-0-MERGES-AT-L-8-AGAINST-72-HERE -- SCOPE=D=2;L=4;BOUNDARY-L=8;FIELD=Q(zeta_8);COINS=640;LINKS=32;PLAQUETTES=16;CARRIER=THE-640-UNIFORM-CONFIGURATIONS;FULL-CONFIGURATION-SPACE=640^32-NOT-A-CARRIER-HERE;WEIGHTS=POSITIVE-EXACT-RATIONALS-NO-LOGS-NO-FLOATS;THE-POTENTIAL-IS-NOMINAL-AND-CARRIED-MULTIPLICATIVELY;CONFINEMENT-VOCABULARY-IS-LICENSED-FOR-THIS-UNIT-ONLY-AND-ONLY-IN-MEASURED-SENTENCES;NO-CONTINUUM-CLAIM;NO-SI-NUMBERS;NOT-QCD
```

(The string is one line; the gate compares that complete string.)

---

## 1. The question, and the object that had to be built first

R5 named three objects a follow-on would need, and ACT priced two of them.
The third it recorded as missing, in its own words, quoted here from its
pinned bytes:

> a family of loops whose size can grow — is recorded as **still absent**, and it is the gate on everything beyond

R5 had said the same thing when it named the gap:

> a family of loops whose size can grow

This unit builds that object, gates it, and then asks the question the pin
put: does the static potential on this arena behave like an area law or like
a perimeter law, and does the answer partition the inventory of 135 couplings
ACT handed over? Everything is carried multiplicatively — expectations are ratios
of exact rational sums, the potential is nominal, and nothing here takes a
logarithm.

**The vocabulary wall, and how it is enforced.** The confinement vocabulary is
licensed for this unit only, and the licence has teeth: every sentence of this
paper that uses one of the licensed words is required, by a gate on the
delivered text, to carry a value this run measured. Unmeasured talk of that
kind stops the delivery run. The pin's must-nots are swept over this paper's
own text with the declaring sentences removed first: this unit makes no
continuum claim and publishes no SI number.

## 2. The arena and the carrier, declared as data (§15)

The stage is R5's, rebuilt here rather than quoted. The lattice is
$(\mathbb{Z}_L)^2$ at $L=4$ and $d=2$: 16 sites, 32 links and 16 plaquettes,
the link and plaquette sets derived from the lattice. The coefficient alphabet
returns 25 elements and the coin family, enumerated exhaustively over the
admissible rows, returns 640 coins splitting into 64 diagonal, 64 antidiagonal
and 512 balanced, every one confirmed unitary by a second route. The carrier is
the parent's own: the 640 uniform configurations, one coin repeated on every
link. The link operator is R5's, quoted from its bytes:

> acts as $U$ on the two-dimensional span of the link's own domino and as the identity on every other site

and the holonomy is R5's too:

> the holonomy is the ordered product of the four link operators around the boundary, each inverted where the boundary runs against the link's own direction

with the consequence that makes the census finite work:

> the whole holonomy lives in a four-by-four block

For a general loop the same argument gives a block on the loop's own sites,
and that block is what this unit's observable is a trace of.

**The classes and the extreme points, rebuilt.** The residual gauge group of
the torus gives the parent's orbits and the full link-stencil gauge image
gives the induced classes: 136 classes against 208 parent orbits, 72 pairs
merged, and the classes are measured to coarsen the orbits. The extreme points
of the reachable set are the uniform measures on those classes — 64 vertices
and 72 edge midpoints, exactly the parent's own split.

**The declared observable is the parent's admissible object.** ACT's repair
made the admissible Wilson weights the conjugation-symmetric functions of the
trace, and this unit carries that forward: the observable is the
conjugation-symmetric part of the loop trace, which lies in the real subfield
by construction. Nine of the 18 declared shapes have a raw trace that is
already real at every coin, so on those the restriction costs nothing; on the
other nine it is doing work, and section 10 measures exactly what.

## 3. The loop family

The family is enumerated with **no silent cap**. The rectangle extents are
swept to the lattice period itself and the simple circuits are *measured*
rather than assumed: a circuit is simple when it closes, visits each of its
links once and each of its sites once.

| a | b | steps | distinct links | distinct sites | simple |
|---|---|---|---|---|---|
| 1 | 1 | 4 | 4 | 4 | yes |
| 1 | 2 | 6 | 6 | 6 | yes |
| 1 | 3 | 8 | 8 | 8 | yes |
| 1 | 4 | 10 | 9 | 8 | no |
| 2 | 1 | 6 | 6 | 6 | yes |
| 2 | 2 | 8 | 8 | 8 | yes |
| 2 | 3 | 10 | 10 | 10 | yes |
| 2 | 4 | 12 | 10 | 9 | no |
| 3 | 1 | 8 | 8 | 8 | yes |
| 3 | 2 | 10 | 10 | 10 | yes |
| 3 | 3 | 12 | 12 | 12 | yes |
| 3 | 4 | 14 | 11 | 10 | no |
| 4 | 1 | 10 | 9 | 8 | no |
| 4 | 2 | 12 | 10 | 9 | no |
| 4 | 3 | 14 | 11 | 10 | no |
| 4 | 4 | 16 | 8 | 7 | no |

So the rectangle ladder at this lattice has extents one, two and three and no
more: the circuits are simple at 9 of the 16 sizes swept, and at the period
itself the opposite legs of the rectangle coincide. To those 144 rectangle
placements the family adds the straight winding cycles at each wrap count and
the staircase — 192 placements and 192 distinct loops, of which
144 contractible and 48 winding, carrying the winding classes the torus has.

**Distinctness is measured under the acting group**, at both declared
readings. A loop's canonical name is least over the rotations of its site
cycle and of its reversal, and that admission is earned rather than declared:
the reversed loop's holonomy is the inverse, whose trace is the conjugate, and
the conjugation-symmetric part is measured equal at every shape and every
coin — 11520 reversed-loop comparisons, no disagreement. The family is
required to be closed under the anchored chart group, and the extension
reading's escapes are published rather than absorbed.

| reading | chart order | distinct loops | orbits | images outside the declared family |
|---|---|---|---|---|
| ANCHORED | 32 | 192 | 11 | 0 |
| EXTENSION | 128 | 192 | 11 | 16 |

There are therefore 11 orbits under a chart group of order 32 — six rectangle
shapes, four winding cycles and the staircase — and the extension reading is
not closed on this family: it carries 16 images outside the declared family,
the staircases of the other handedness, and that is a fact about the declared
family rather than about the arena.

**The window is named in the verdict**: rectangle circuits together with
straight and staircase winding cycles, and not all closed paths. A loop that
backtracks, or wanders, is outside what this unit declares.

**The observable is base-point blind, and that is a measurement**: the value
at one placement equals the value at every other, over 111360
placement-by-coin comparisons with no mismatch. It is what makes an
expectation over the family a function of the shape and not of where the loop
was put. And the one member the parents had already measured is reproduced
from these primitives before anything new is said: the plaquette's trace takes
11 distinct values on the carrier, its counting expectation is 13/10, and 56
of the diagonal coins are non-flat, which is the grandparent's own count.

## 4. The discriminator, defined before any row runs

Three legs, all multiplicative, all exact.

**LEG-AREA.** At equal perimeter, does the expectation move with the area? The
ladder shapes are grouped by perimeter and the pairs with equal perimeter and
unequal area are the comparisons; an area law would show itself at those
comparisons without a model, and this unit takes them at every one of the
136 extreme points as well as at the declared rows.

**LEG-CREUTZ.** The Creutz-ratio analogue
$\chi(a,b) = W(a,b)\,W(a-1,b-1) / \big(W(a,b-1)\,W(a-1,b)\big)$, which is one
for a law geometric in the perimeter and constant and not one for a law
geometric in the area. Every value is an exact element of the real subfield.

**LEG-WINDING.** The winding-cycle expectation, as the analogue of a
deconfinement order parameter: zero or not zero, a two-valued verdict, and
section 7 reports it at all 136 extreme points.

**The discriminator's own well-definedness is itself a gated measurement**, in
the pin's words:

> The discriminator's own well-definedness (nonzero denominators; enough ladder rungs at the declared L) is itself a gated measurement

and it is taken before a single expectation is computed. At this lattice the
ladder carries 2 area-discriminating comparisons and 4 Creutz rungs. That is
thin, and it is published as thin: had it carried neither, the honest verdict
would have been that the discriminator is degenerate at this lattice and no
row could have rescued it.

**The L-boundary is stated as a measured law and not as a quoted one.** The
pin binds it:

> the merging-index law 8/gcd(L,8) means family-level pinning dies at L ≡ 0 mod 8

and ACT states the same law in its own bytes:

> The merging index is $8/\gcd(L,8)$, and at every L divisible by eight it is 1, and there the price is not reduced, no orbit pair is identified and no observable is pinned

| L | residual order on the torus | the link stencils gauge image | the merging index | the odd twist is a gauge transformation here |
|---|---|---|---|---|
| 2 | 2 | 8 | 4 | no |
| 4 | 4 | 8 | 2 | no |
| 8 | 8 | 8 | 1 | yes |
| 16 | 8 | 8 | 1 | yes |

Every claim this unit makes about the class structure therefore carries its
lattice scope, and section 11 runs the boundary rather than projecting it.

## 5. The ladder is a function of the perimeter alone

This is the unit's deepest measurement and it is taken at the level of
configurations, not of expectations. At every coin of the carrier and at every
pair of ladder shapes of equal perimeter, the loop observable takes the same
value: 5760 shape-by-coin comparisons at equal perimeter and 0 disagreements,
so **the area does not enter it**. Because the statement is per-configuration, it
holds under every measure on this carrier without a second census — at every
declared row, at every extreme point, and at every point of the allowed space
between them.

The mechanism is visible once the loop is read as a word. On a uniform
configuration every link carries the same coin, so the holonomy of a circuit
depends only on the cyclic sequence of orientations around it; for an
$a \times b$ rectangle that sequence is $a+b$ forward steps followed by $a+b$
backward steps, which depends on $a$ and $b$ only through their sum. The
direction of a leg cannot matter because the coin does not know which
direction it sits in.

**And the dependence on the perimeter has an exact closed form.** Writing
$P = a+b$, the ladder is fitted exactly from three points to
$W = A + B\,P + C\,2^{-P}$ and then verified at every remaining point at every
coin: 0 closed-form failures over 640 coins. That is an over-determined test
and not a fit — three coefficients against five ladder points, at each of the
640 coins. The three coefficients are rigid: the halving coefficient takes
exactly two values over the carrier, and it is the indicator of the balanced
sector — one at all 512 balanced coins and zero at the 64 diagonal and the 64
antidiagonal ones.

Read as a transfer object, that closed form is a spectrum: a constant mode, a
mode proportional to the perimeter, and a mode halving with it. The
eigenvalue one appears twice because a term proportional to the perimeter is a
two-block at eigenvalue one, so the transfer object this ladder sees has
spectrum {1,1,1/2} and gap 1/2.

## 6. Mode (a): the expectations at the declared rows

Four weight systems are declared and each is stamped
`CONDITIONAL-ON-THE-DECLARED-WEIGHTS` on its own row. Each plaquette
expectation is required to equal the parent's own published number at a named
receipt path before any larger loop is believed: 13/10 at THE-NULL,
262244/65615 at THE-WILSON-CONJUGATION-SYMMETRIC and
4294967399/4294967375 at THE-EXHIBITED-WITNESS. The law-native row is the
CONTROL and its value is this unit's own measurement of a transported law:
225/152 at THE-LAW-NATIVE-CONTROL. Its stamp is carried verbatim from the
parent's receipt and it is **never spent as derived** — the parent's order is
explicit in its own bytes:

> ACT must not treat it as a derived measure and must not spend it as

| row | weight system | exponent | plaquette expectation | ladder word | winding word |
|---|---|---|---|---|---|
| THE-NULL | W-IDENTICALLY-ONE | 32 | 13/10 | DISCRIMINATOR-DEGENERATE | WINDING-ORDER-ZERO |
| THE-WILSON-CONJUGATION-SYMMETRIC | TWO-AT-THE-TOP-TRACE-VALUE-AND-ONE-ELSEWHERE | 16 | 262244/65615 | DISCRIMINATOR-DEGENERATE | WINDING-ORDER-NONZERO |
| THE-EXHIBITED-WITNESS | TWO-ON-ONE-CLASS-AND-ONE-ELSEWHERE | 32 | 4294967399/4294967375 | DISCRIMINATOR-DEGENERATE | WINDING-ORDER-ZERO |
| THE-LAW-NATIVE-CONTROL | THE-TRANSPORTED-SECTOR-LAW | 1 | 225/152 | DISCRIMINATOR-DEGENERATE | WINDING-ORDER-ZERO |

The ladder leg returns the same word at all four rows: the area is not seen
and the Creutz ratio is not one, so neither shape of law is present. The
winding leg does not: 1 of the 4 declared rows carries a non-zero winding
expectation, and it is the Wilson row. Three declared measures kill the
winding-cycle expectation exactly; one does not.

The three coefficients make the difference legible, and they are the
transfer content of each row.

| row | mode A | mode B | mode C | closed form holds |
|---|---|---|---|---|
| THE-NULL | 11/10 | 0 | 4/5 | yes |
| THE-WILSON-CONJUGATION-SYMMETRIC | 8/5965 | 26214/13123 | 64/65615 | yes |
| THE-EXHIBITED-WITNESS | 4294967383/4294967375 | 0 | 64/4294967375 | yes |
| THE-LAW-NATIVE-CONTROL | 53/38 | 0 | 13/38 | yes |

The null, the witness and the control all have a vanishing
perimeter-proportional coefficient; the Wilson row does not, because the
weight it declares raises exactly the flat coins, and a flat holonomy
contributes its own block dimension to the trace. The gap is the same at every
row: what the coupling moves is the amplitude, not the spectrum.

## 7. Mode (b): the family sweep

Every extreme point of the reachable set is a declared measure and every one
is run — 136 extreme points swept, 64 vertices and 72 edge midpoints — with no
sampling anywhere. The integers in this section are counts and the receipt
stamps them so: no count becomes a probability without a declared measure
(E-24).

The area leg returns the same value at all of them, which is what the
per-configuration theorem of section 5 requires. The Creutz leg does not:
there are 24 of the 136 extreme points where the ratio is one, and they are
exactly the points at which a single mode is active, while the rest carry two
modes or three. The winding leg does not either: 96 with a non-zero winding
expectation and 40 with zero.

**The interior is sampled as well as the corners.** Nine exact one-parameter
slices — one class raised to three declared exact rational couplings — return
the same area verdict while their expectations move, so the sweep's finding is
not a property of the corners alone: 9 exact one-parameter slices, three
distinct plaquette expectations, and the area leg unmoved at every one.

## 8. The price binding

The question the pin put is whether the discriminator's verdict partitions the
inventory ACT handed over. It is answered leg by leg and by counting, never by
argument. A coupling is a coordinate of the allowed space modulo
normalisation, so fixing the first class as the base leaves one coupling per
remaining class — the inventory of 135 couplings — and each coupling's strong
limit is the extreme point on its own class.

| leg | distinct values over the extreme points | partitions | tally over the 136 extreme points | verdict |
|---|---|---|---|---|
| LEG-AREA | 1 | no | False 136 | FAMILY-INVARIANT |
| LEG-CREUTZ-WORD | 2 | yes | DECONFINES 24, DISCRIMINATOR-DEGENERATE 112 | PARTITIONS-THE-INVENTORY |
| LEG-CREUTZ-UNIT | 2 | yes | False 112, True 24 | PARTITIONS-THE-INVENTORY |
| LEG-CREUTZ-DEFINED | 1 | no | True 136 | FAMILY-INVARIANT |
| LEG-WINDING | 2 | yes | WINDING-ORDER-NONZERO 96, WINDING-ORDER-ZERO 40 | PARTITIONS-THE-INVENTORY |
| MODE-A-CONSTANT | 8 | yes | 0 16, 1 24, 1+-1/2*sqrt2 16, 1+1/2*sqrt2 16, 2 24, 2+-1*sqrt2 16, 2+1*sqrt2 16, 4 8 | PARTITIONS-THE-INVENTORY |
| MODE-B-PERIMETER | 9 | yes | -1 8, -2 8, 0 40, 0+-1*sqrt2 16, 0+-1/2*sqrt2 16, 0+1*sqrt2 16, 0+1/2*sqrt2 16, 1 8, 2 8 | PARTITIONS-THE-INVENTORY |
| MODE-C-HALVING | 2 | yes | 0 72, 1 64 | PARTITIONS-THE-INVENTORY |
| ACTIVE-MODES | 3 | yes | 1 32, 2 64, 3 40 | PARTITIONS-THE-INVENTORY |

**The area leg partitions none of them.** No coupling this arena admits can
make the loop observable see the area, because the blindness is a property of
the carrier and not of the measure. Whether the Creutz ratio is defined is
family-invariant too, and it is undefined at 0 of them.

**Every other leg partitions the inventory**, exactly and by counting. The
halving mode is absent at 72 of the 136, which is precisely the union of the
diagonal and antidiagonal classes; the perimeter-proportional mode is absent
at 40 of them; and the winding leg splits the family 96 against 40. So the
answer to the pin's question is two-sided and both sides are measured: the
confinement discriminator's area leg is family-invariant at this arena, while
the order-parameter leg and the amplitudes are coupling-dependent.

**And over the 136 extreme points the order parameter is not pinned by the
action route.** ACT's falsifier asks whether a locality declaration narrows an
observable's range; put to the winding expectation, the range over the
reachable set is measured equal to the range over the invariant simplex and to
the observable's own range, all three running from -4 to 4. What the locality
declaration pins elsewhere it does not pin here.

## 9. The spectral door

The door itself **is named and not run**, with its cost in the open. The
transfer matrix of this arena lives on a time slice of the full configuration
space; a slice is the four links of one row, which is 167772160000 states per
time slice, and the matrix has that number squared entries. It is out of reach
by cost at this arena and it is named rather than approximated.

**The finite form the window does reach is measured**, and it is what SPC
inherits. The ladder's own transfer content is the closed form of section 5:
spectrum {1,1,1/2} and gap 1/2, verified at every coin of the carrier and
unchanged at every declared row. The gap is family-invariant; what a coupling
moves is which modes are switched on and how strongly.

**And the holonomy's own spectrum is measured too, so that where the decay can
live is settled rather than assumed.** The winding cycle's power-sum ladder
gives the characteristic polynomial of the winding holonomy by Newton's
identities in exact arithmetic; its determinant is required to have modulus
one and its coefficient sequence to be self-inversive, and both hold at
640 of 640 holonomies. The winding holonomy is unitary, its spectrum lies on
the unit circle, and it therefore **carries no decaying direction**. The decay
this arena has is the ladder's halving mode and nothing else.

## 10. The orientation reading, declared and priced

The chart group contains orientation-reversing elements, and they are what
make the conjugation-odd part of a loop trace unlawful: a reflection sends a
loop to its reverse, whose holonomy is the inverse, whose trace is the
conjugate. Declaring the **oriented** subgroup instead is a DECLARATION and
never a derivation. Neither standard is claimed true here; the verdict for
this axis is the declaration-relativity itself, and every row carries the
stamp `DECLARATION-RELATIVE-AT-THE-ORIENTATION-READING`.

**What the declaration is.** The oriented subgroup is the chart group minus
its orientation-reversing elements, and the index of the oriented subgroup is
2 at both readings — one binary choice of convention, purchased and named.

**What it costs, in the pricing frame.** The form census at the link grain is
re-run at all four readings.

| reading | chart group order | the link stencils chart stabilizer | orbits | coupling count |
|---|---|---|---|---|
| ANCHORED | 32 | 1 | 136 | 135 |
| ANCHORED-ORIENTED | 16 | 1 | 136 | 135 |
| EXTENSION | 128 | 4 | 80 | 79 |
| EXTENSION-ORIENTED | 64 | 2 | 80 | 79 |

At the anchored reading the link stencil's chart stabiliser is already
trivial, so dropping the reflections changes nothing there:
135 couplings at the anchored reading and 135 at the oriented one. At the
extension reading the stabiliser halves, from four to two, and yet the acting
group on a single link's coin does not — the two elements that survive still
induce both the identity and the swap conjugation on a single link's coin,
which is all this census sees — so the orbit count and the coupling count are
unchanged there as well. **The declaration buys
observables and costs no coupling**, and that is the price, measured rather
than argued.

**What it buys.** The conjugation split is exact: every trace is split into
its symmetric and odd parts and the two are required to reconstruct it in the
full field at every shape and every coin, with no failure. The odd part is
measured to change sign under the reversal a reflection performs, at every
shape and every coin, which is exactly why it is unlawful at the full standard
and lawful at the oriented one. It is not a small effect:
3760 of 11520 shape-by-coin traces carry a non-zero odd part at 9 of the 18
shapes — and those nine are precisely the winding family, the straight cycles
at every wrap count and the staircase. The contractible rectangles carry none.

**And what it buys is invisible to every measure this unit declared.** At all
four declared rows the odd observables vanish identically: 0 non-zero odd
observables at every declared row, because each declared weight system is
constant on the coin sectors or on the classes, and the odd part sums to zero
over each of them. On the family it is another story: 96 of the 136 extreme
points carry at least one. So the oriented declaration adds a sector of
observables that the arena's natural rows cannot see and most of the allowed
space can.

## 11. The L-boundary, run

The boundary is **run and not projected**. At the smallest lattice size where
the merging index is one, the loop legs are re-measured.

| quantity | at the declared L | at the boundary L |
|---|---|---|
| simple rectangle shapes | 9 | 49 |
| area-discriminating comparisons | 2 | 70 |
| the longest ladder perimeter | 6 | 14 |
| the merging index | 2 | 1 |
| the parents orbits | 208 | 136 |
| the induced classes | 136 | 136 |
| orbit pairs merged | 72 | 0 |

Three findings **survive the boundary** and one does not, and both halves are
measurements. Area-blindness survives:
44800 comparison-by-coin checks at L = 8 with 0 disagreements, over a set of
comparisons that is far richer there than here. The closed form survives, and
survives in the strongest possible sense —
640 of the 640 coins carry identical coefficients at the two lattice sizes,
over a ladder that runs to a perimeter of fourteen rather than six — so the
spectrum and the gap are lattice-invariant across the declared pair. What does
not survive is the class merging: 0 merges at L = 8 against 72 here, which is
the merging-index law doing exactly what ACT said it would. Every count in
sections 7 and 8 is therefore scoped to lattices whose size the phase order
does not divide, while the area leg's verdict and the spectral content are
not.

## 12. What could have answered the other way

Five outcome words were pre-registered and the head law is required to return
each of them from a measurement rather than from an assertion. The pattern is
the parent's: declared synthetic ladders are driven through the SAME
discriminator and the SAME head law the delivered rows go through, with no
delivered field overwritten and no row forged.

| arm | the synthetic law | ladder word | head word | agrees |
|---|---|---|---|---|
| SYN-AREA-LAW | A-LAW-GEOMETRIC-IN-THE-AREA | CONFINES | POT-CONFINES-AT | yes |
| SYN-PERIMETER-LAW | A-LAW-GEOMETRIC-IN-THE-PERIMETER | DECONFINES | POT-DECONFINES-AT | yes |
| SYN-ZERO-DENOMINATOR | A-LADDER-WITH-A-VANISHING-RUNG | DISCRIMINATOR-DEGENERATE | POT-DISCRIMINATOR-DEGENERATE-AT | yes |
| SYN-MISSING-SHAPE | A-LADDER-THE-FAMILY-DOES-NOT-COVER | BLOCKED-AT-THE-LADDER | POT-BLOCKED-AT | yes |
| SYN-TWO-ROWS-THAT-DISAGREE | ONE-AREA-ROW-BESIDE-ONE-PERIMETER-ROW | CONFINES|DECONFINES | POT-SPLIT-BY-WEIGHT | yes |

A law geometric in the area returns a constant Creutz ratio of one half and
the area seen, and the selector emits its word; a law geometric in the
perimeter returns a Creutz ratio of one at every rung, and the selector emits
the other; a ladder with a vanishing rung has no defined ratio and returns the
degenerate word; a ladder the family does not cover returns the blocked word;
and two rows that disagree return the split. So all 5 control arms come back
with the word they were built to produce, and the head this unit delivers is
the one its own measurement selected out of a live field.

## 13. The instrument

The instrument is `v14/code/pot_exact.py` and its contract is the era's
minimum (#82): a delivery run that is the only writer, a `--no-write` twin, a
falsification self-test that corrupts one anchor class in memory, restores it
before the next, and requires each to die at that class's own gate writing
nothing, a per-mutant runner, an all-mutants sweep, gate and mutant
listings, and a `--verify-paper` mode. Unknown flags exit 2. The exit
conventions invert the usual reading and are disclosed in the usage string, in
the receipt and here: the delivery run exits 0 on success and 1 on any
refusal, writing nothing; `--selftest` exits 0 when every anchor class is
fatal; `--mutant` exits 0 when the named mutant *dies* on its declared target.

The gate ledger is chained row by row and the chain is verified in the run and
again at the disk boundary from the bytes read back. 37 gates close before the
paper gates; the paper gates and the consumer register follow them; and two
closing gates come last — the seal and the artifact integrity check, which
cannot be inside the ledger they close over. There are 49 declared mutants,
each at the gate it was declared to falsify; that sweep is an external-battery
result and the receipt says so rather than implying the delivery run produced
it. The registry is checked TOTAL against the instrument's own syntax tree, so
a falsifier cannot exist as an unswept branch and a switch the scan cannot
read is fatal rather than forgiven; the function inventory is checked total the
same way. Each mutant's published description names the exact token it plants
and that token is located in the source text (E-23). The coverage is published
at an honest denominator (#34) whose denominator is every gate on the clean
path and not only the ones already closed: each is either a declared
falsifier's target or carries a registered forcing.

The paper gates run in the plain delivery run and not in a separate mode
(#20): claim rendering at exact occurrence counts, the complete verdict string
by equality with the fenced blocks compared as a multiset (E-22), claim
polarity over the direction-bearing headlines, the structural binding of every
table header as a claim rendered from the receipt keys its own columns are,
numeral coverage over every numeral including the fenced verdict block and
both sides of every fraction, the spelled-numeral scan above twelve, the
must-not sweep with the declaring sentences removed first, the table-row
coverage, and the requirement that every quotation lie inside a pinned
verbatim window. 19 objects are sealed before the paper gates, and the
manifest is total.

**Deviations, and the register of scope.** The pin's arena, gates and
must-nots are followed as written. Three points are recorded as scope rather
than deviation. First, the loop family is a **declared window** and not the
set of all closed paths: rectangle circuits together with straight and
staircase winding cycles, named in the verdict, closed under the anchored
chart group and carrying 16 images outside the declared family at the
extension reading. Second, the boundary lattice re-runs the loop legs and the
class structure and nothing else — no form census is run at that size, and the
counts it reports are the ones this unit measured there rather than a second
inventory. Third, the odd-observable sweep at the extreme points is reported
as a count of points carrying at least one non-zero value rather than as a
table of every value, which would be 136 rows against a family of 18 shapes;
the values themselves are published for the declared rows, where they are
0 non-zero odd observables at every declared row.

## 14. What this decides, and what it does not

**Decided, at the declared scope.**

- **The loop family exists, and its extent is measured.** 192 placements and
  192 distinct loops, 144 contractible and 48 winding, 11 orbits under a chart
  group of order 32, closed under the anchored chart group and carrying
  16 images outside the declared family at the extension reading. The window
  is named in the verdict and there is **no silent cap** anywhere in the
  enumeration.
- **The loop observable is a function of the perimeter alone**, at every
  configuration of the carrier and hence under every measure on it, with an
  exact closed form in three modes verified over-determinedly at every coin.
- **The area leg of the discriminator is therefore family-invariant**, and the
  partition it induces on the inventory of 135 couplings is trivial. This is
  the finite, exact sense in which this arena cannot exhibit an area law,
  measured at 5760 comparisons: not that the effect is small, but that the
  observable cannot see the area at all.
- **The other legs partition the inventory**, exactly: the winding leg
  96 against 40, the halving mode absent at 72 of the 136, the
  perimeter-proportional mode absent at 40 of them, and the Creutz leg at one
  precisely on the single-mode corners.
- **The transfer content is measured, and the door is named.** The finite form
  is spectrum {1,1,1/2} and gap 1/2; the true transfer matrix at
  167772160000 states per time slice is named and not run; the winding
  holonomy is unitary at 640 of 640 holonomies and carries no decaying
  direction.
- **The orientation reading is a declaration with a measured price.** The
  index of the oriented subgroup is 2 at both readings; the coupling inventory
  does not move; a sector of odd observables becomes lawful, non-zero at
  3760 of 11520 shape-by-coin traces carry a non-zero odd part at 9 of the 18
  shapes, invisible at every declared row and visible at 96 of the 136 extreme
  points.

**Not decided, and named.**

- **The full configuration space.** Everything here is measured on the
  parent's carrier of uniform configurations. What a non-uniform carrier would
  do to the perimeter-only finding is untouched, and the mechanism's own
  premise — one coin on every link — is exactly what a non-uniform carrier
  removes. This is the first successor this unit registers.
- **The true transfer matrix.** It is named with its cost and not run, and no
  statement about its spectrum is made here.
- **Which standard of orientation is the right one.** The unit prices the
  declaration and does not make it.
- **Whether the loop family should be larger.** The declared window is
  rectangle circuits and straight and staircase winding cycles; a family
  closed under the extension reading would need the 16 escaping staircases,
  and what a wider family would show is not decided here.
- **Anything about a limit.** The two lattice sizes this unit runs are
  declared points, not a sequence, and no statement is made about any other.
