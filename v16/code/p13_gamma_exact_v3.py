...
FRESH_DOMAIN = b"P13-TYPED-GROUPOID-FRESH-v3"
...
def tensor_action_row(
    ...
    if left_node not in boundary_nodes(left_witness.source) or right_node not in boundary_nodes(
        ...
        raise Refusal("tensor factor node is absent from its witness source")
...
class CertificateActionInput:
    ...
    def __post_init__(self) -> None:
        ...
        coefficient = map_coefficient(
            evaluate_arrow(self.law, self.arrow), self.target_row, self.source_column
        )
...
def build_certificate_action_input(
    ...
    if canonical_bytes(rebuilt) != canonical_bytes(inherited_certificate):
...
def rebuild_certificate_input(
    ...
    return build_certificate_action_input(
        law,
...
class CertificateTransportTriple:
    ...
    def __post_init__(self) -> None:
        ...
        expected = (
            ...
            and canonical_bytes(self.literal_transport)
            == canonical_bytes(self.independent_rebuild)
        )
...
def certificate_transport_triple(
    ...
    transported = act_certificate(witness, value)
    rebuilt = rebuild_certificate_input(
        value.law,
        witness.target,
        value.generator_ast_address,
    ...
    coordinates = tuple(
        (field, canonical_bytes(getattr(transported, field)) == canonical_bytes(getattr(rebuilt, field)))
        for field in fields
    )
    exact = all(value for _, value in coordinates) and canonical_bytes(
        transported
    ...
    return (
        ...
        and all(
            ...
            and _certificate_triple_reference_exact(
                row.get("identity_triple", {}), identities
            )
            and row["identity_triple"]["original_ref"] == row["input_ref"]
            for row in identity_rows
        )
        and all(
            _certificate_triple_reference_exact(
                family.get("nontrivial_triple", {}), identities
            )
...
def measure_tensor_configuration_action() -> dict[str, Any]:
    left_arrow = _small_generator(
        _small_atom("action_tensor_left_"), "action_tensor_left_"
    )
    right_arrow = _small_generator(
        _small_atom("action_tensor_right_"), "action_tensor_right_"
    )
    left_witness = _suffix_groupoid_witness(
        left_arrow, "__tensor_action", ("role", "matter", "port", "occurrence")
    )
    right_witness = _suffix_groupoid_witness(
        right_arrow, "__tensor_action", ("role", "matter", "port", "occurrence")
    )
    tensor_presentation = presentation_from_arrow(
        tensor_arrow(left_arrow, right_arrow)
    )
    ...
    return {
        ...
        "factor_certificate_lineage": (
            canonical_hash(arrow_key(left_arrow)),
            canonical_hash(arrow_key(right_arrow)),
        ),
        "all_exact": row.exact
        and _complete_byte_store_exact(frozen_store)
        and _configuration_law_reference_exact(reference, identities),
    }
...
def measure_complete_certificate_action_census(law: GammaLaw) -> dict[str, Any]:
    ...
    for family_index, arrow in enumerate(registered_generator_families()):
        ...
        for value in values:
            triple = certificate_transport_triple(identity, value)
            exact = triple.exact and canonical_bytes(
                triple.literal_transport
            ...
            row = {
                ...
                "pairing_key": certificate_pairing_key(value),
                "input_ref": _store_complete_bytes(object_store, value),
                "identity_triple": _certificate_triple_reference(
                    triple, object_store
                ),
                "exact": exact,
            }
        ...
        nontrivial = certificate_transport_triple(nontrivial_witness, values[0])
        family_rows.append(
            {
                "family_index": family_index,
                ...
                "nontrivial_triple": _certificate_triple_reference(
                    nontrivial, object_store
                ),
    ...
    return {
        ...
        "all_nontrivial_family_rows_exact": all(
            row["nontrivial_triple"]["exact"] for row in family_rows
        ),
...
def measure_groupoid_covariance(law: GammaLaw) -> dict[str, Any]:
    ...
    tensor_configuration_action = measure_tensor_configuration_action()
...
def groupoid_promotion_predicate(groupoid: Mapping[str, Any]) -> bool:
    ...
    complete_certificate_ids = _complete_byte_store_identities(
        complete_certificates["complete_input_table"]
    )
    ...
    return (
        ...
        and all(
            identity_row["input_ref"] in complete_certificate_ids
            and _certificate_triple_reference_exact(
                identity_row["identity_triple"], complete_certificate_ids
            )
            and identity_row["identity_triple"]["original_ref"]
            == identity_row["input_ref"]
            for family in complete_certificates["family_rows"]
            for identity_row in family["identity_rows"]
        )
        and all(
            _certificate_triple_reference_exact(
                family["nontrivial_triple"], complete_certificate_ids
            )
            for family in complete_certificates["family_rows"]
        )
        ...
        and bool(tensor_configuration["factor_certificate_lineage"])
...
def _typed_groupoid_v3_action_attack(
    identifier: str, measurements: Mapping[str, Any] | None = None
) -> dict[str, Any]:
    law = GammaLaw(Fraction(1, 2))
    application = _minimal_bound_split_application(law)
    ...
    old: Any = {"lawful_configuration_action": act_configuration(source_node, source_configuration, identity)}
    new: Any = {"attack": identifier}
    evidence: dict[str, Any] = {}
    passed = False
    changed_path = "typed_groupoid_v3.action"
    ...
    if identifier == "NONIMAGE-ACTIVE-CARRIED":
        ...
        census = configuration_action_census(presentation, first, second, third)
        changed = dict(census)
        ...
        if identifier == "MALFORMED-TRANSPORTED-CERTIFICATE-BYTES":
            ...
            census = certificate_action_census(law, presentation, (), first, second, third, "CREATE")
            changed = dict(census)
            rows = list(census["rows"])
            ...
            raise Refusal("unknown v3 typed-groupoid action attack")
...
GROUPID_ATTACKS = (
    "RELABEL-RAW-NAME",
    "RELABEL-ORIENTATION",
) + TYPED_GROUPOID_ATTACKS + V3_TYPED_GROUPOID_ATTACKS
...
def run_attack(
    ...
    if identifier in GROUPID_ATTACKS:
        if identifier in V3_TYPED_GROUPOID_ATTACKS:
            return _typed_groupoid_v3_action_attack(identifier, measurements)
...
ANCHOR_SPECS = (
    (
        "v16/note-paper13-typed-groupoid-source-delta-pin.md",
        SOURCE_DELTA_PIN_SHA256,
        "typed-groupoid-v3-source-delta-contract",
    ),
    (
        "v16/note-paper13-typed-groupoid-source-audit-adjudication.md",
        SOURCE_AUDIT_ADJUDICATION_SHA256,
        "typed-groupoid-v2-source-audit-adjudication",
    ),
    (
        "v16/code/p13_gamma_exact_v2.py",
        REJECTED_V2_SOURCE_SHA256,
        "rejected-v2-source-regression-only-never-imported",
    ),
    (
        "v16/note-paper13-typed-groupoid-source-freeze.md",
        REJECTED_V2_FREEZE_SHA256,
        "rejected-v2-source-freeze-authentication-only",
    ),
    (
        "v16/review-paper13-typed-groupoid-source-operator.md",
        V2_OPERATOR_SOURCE_REPORT_SHA256,
        "v2-source-operator-counterexample",
    ),
    (
        "v16/review-paper13-typed-groupoid-source-records.md",
        V2_RECORDS_SOURCE_REPORT_SHA256,
        "v2-source-records-counterexample",
    ),
    (
...
def _read_ledger_selfcheck() -> dict[str, Any]:
    ...
    official_rows = (
        {
            "open_index": 1,
            "path": "v16/code/p13_gamma_fresh_cases_v3.json",
    ...
    return {
        ...
        == "v16/code/p13_gamma_fresh_cases_v3.json"
        and official_rows[1]["path"] == "v16/code/p13_gamma_exact_v3.py",
...
def run_selftest() -> dict[str, Any]:
    ...
    checks = (
        ...
        _selftest_check(
            ...
            len(MUTANT_IDS) == 149
            and len(MUTANT_IDS) - len(V3_TYPED_GROUPOID_ATTACKS) == 112
...
def build_receipt(
    ...
    artifacts = {
        ...
        "rejected_v2_source_cycle": {
            "source_sha256": REJECTED_V2_SOURCE_SHA256,
            "freeze_sha256": REJECTED_V2_FREEZE_SHA256,
            "operator_report_sha256": V2_OPERATOR_SOURCE_REPORT_SHA256,
            "records_report_sha256": V2_RECORDS_SOURCE_REPORT_SHA256,
...
def generate_fresh_mode(parsed: Mapping[str, str]) -> dict[str, Any]:
    ...
    _validate_publication_role(
        destination, "v16/code/p13_gamma_fresh_cases_v3.json"
    )
...
def run_official_mode(parsed: Mapping[str, str]) -> dict[str, Any]:
    ...
    _validate_publication_role(
        fresh_path, "v16/code/p13_gamma_fresh_cases_v3.json"
    )
    _validate_publication_role(output_path, "v16/code/p13_gamma_output_v3.txt")
    _validate_publication_role(receipt_path, "v16/code/p13_gamma_receipt_v3.json")
