...
FRESH_DOMAIN = b"P13-TYPED-GROUPOID-FRESH-v2"
...
def _native_groupoid_law_row(
    ...
    transformed_source_state = relabel_configuration(
        arrow.source, transformed_arrow.source, source_state, first
    )
    ...
    if certificate_operation is not None:
        ...
        original_certificates = tuple(
            certificate
            for certificate in _all_generator_split_certificates(law, arrow)
            if certificate.operation_kind == certificate_operation
        )
        transformed_certificates = tuple(
            certificate
            for certificate in _all_generator_split_certificates(
                law, transformed_arrow
            )
            if certificate.operation_kind == certificate_operation
        )
        certificate_row = {
            "operation": certificate_operation,
            "original_count": len(original_certificates),
            "transformed_count": len(transformed_certificates),
            "original_classifier_hashes": tuple(
                certificate.classifier_consumed_sha256
                for certificate in original_certificates
            ),
            "transformed_classifier_hashes": tuple(
                certificate.classifier_consumed_sha256
                for certificate in transformed_certificates
            ),
            "all_original_final": bool(original_certificates)
            and all(certificate.final for certificate in original_certificates),
            "all_transformed_final": bool(transformed_certificates)
            and all(certificate.final for certificate in transformed_certificates),
            "count_preserved": len(original_certificates)
            == len(transformed_certificates),
        }
    ...
    law_fields = {
        ...
        or (
            certificate_row["all_original_final"]
            and certificate_row["all_transformed_final"]
            and certificate_row["count_preserved"]
        ),
...
def groupoid_promotion_source_scan(source: str) -> dict[str, Any]:
    ...
    return {
        ...
        "ast_sha256": canonical_hash(ast.dump(function, include_attributes=False)),
        "all_exact": required_present and copied_final_absent,
    }
...
def _read_ledger_selfcheck() -> dict[str, Any]:
    ...
    official_rows = (
        {
            "open_index": 1,
            "path": "v16/code/p13_gamma_fresh_cases_v2.json",
    ...
    return {
        ...
        == "v16/code/p13_gamma_fresh_cases_v2.json"
        and official_rows[1]["path"] == "v16/code/p13_gamma_exact_v2.py",
...
def run_selftest() -> dict[str, Any]:
    ...
    checks = (
        ...
        _selftest_check(
            ...
            groupoid_promotion_predicate(measurements["groupoid"]),
            measurements["groupoid"],
        ),
        _selftest_check(
            ...
            {
                "abstract": measurements["groupoid"]["abstract_census"],
                "native": measurements["groupoid"]["native_census"],
            },
        ),
        _selftest_check(
...
def generate_fresh_mode(parsed: Mapping[str, str]) -> dict[str, Any]:
    ...
    _validate_publication_role(
        destination, "v16/code/p13_gamma_fresh_cases_v2.json"
    )
...
def run_official_mode(parsed: Mapping[str, str]) -> dict[str, Any]:
    ...
    _validate_publication_role(
        fresh_path, "v16/code/p13_gamma_fresh_cases_v2.json"
    )
    _validate_publication_role(output_path, "v16/code/p13_gamma_output_v2.txt")
    _validate_publication_role(receipt_path, "v16/code/p13_gamma_receipt_v2.json")
