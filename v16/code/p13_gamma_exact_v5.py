...
REJECTED_V4_SOURCE_SHA256 = (
    "272b64972e3eb620867cf6ad0decf8db1fcc03092adfd1d69d77aa10e2605910"
)
REJECTED_V4_FREEZE_SHA256 = (
    "5bf797eae6a80690a27bcad59c8e3b2206f008f1ce2039d4b6883d22a82d38c7"
)
V4_OPERATOR_SOURCE_REPORT_SHA256 = (
    "59c5fa4206df7f7e2c9df9f820fb1256a6e3a6d73af2fac238a64c6ed336a5c4"
)
V4_RECORDS_SOURCE_REPORT_SHA256 = (
    "8f6faca179c0d13331d69d683db837bc2f050d573ff139ce96ba41ca31b837cc"
)
...
FRESH_DOMAIN = b"P13-TYPED-GROUPOID-FRESH-v5"
...
def _complete_byte_store_view(
    entries: Sequence[Mapping[str, Any]],
) -> tuple[bool, frozenset[str], dict[str, bytes]]:
    """Authenticate a store once and retain its complete explicit preimage."""
    ...
    if not unique:
        return False, frozenset(), {}
    return True, frozenset(identities), payloads


def _complete_byte_store_exact(entries: Sequence[Mapping[str, Any]]) -> bool:
    return _complete_byte_store_view(entries)[0]


def _complete_byte_store_identities(
    entries: Sequence[Mapping[str, Any]],
) -> set[str]:
    exact, identities, _ = _complete_byte_store_view(entries)
    if not exact:
        return set()
...
def _complete_byte_store_payloads(
    ...
    exact, _, payloads = _complete_byte_store_view(entries)
    if not exact:
        return {}
    return payloads
...
def _measure_complete_action_raw_verification(
    ...
    for law_kind, family_index, row_index, action, input_ref, pairing_key in (
        ...
        row_input_cache: dict[str, CertificateActionInput] = {}
        row_pairing_cache: dict[str, tuple[Any, ...]] = {}
        ...
        if decoded_exact:
            seen_input_refs.update(row_input_cache)
        evidence.append(
            {
                "law_kind": law_kind,
    ...
    return {
        ...
        "decoded_input_count": len(seen_input_refs),
...
def _configuration_action_census_exact(census: Mapping[str, Any]) -> bool:
    if type(census) is not dict:
        return False
    store_exact, frozen_identities, _ = _complete_byte_store_view(
        census.get("complete_object_table", ())
    )
    if not store_exact:
        return False
    identities = set(frozen_identities)
...
def _tensor_configuration_action_exact(census: Mapping[str, Any]) -> bool:
    ...
    if type(cases) is not tuple:
        return False
    raw_verification = _measure_tensor_lineage_raw_verification(
        cases, census["complete_object_table"]
    )
    flattened_lineage = tuple(
        entry for case in cases for entry in case.get("factor_certificate_lineage", ())
    )
    cardinalities = tuple(
        len(case.get("factor_certificate_lineage", ())) for case in cases
    )
    return (
        census.get("case_count") == len(cases) == 3
        and census.get("factor_certificate_lineage") == flattened_lineage
        and census.get("lineage_cardinalities") == cardinalities
        and census.get("raw_lineage_verification") == raw_verification
        and census.get("independently_rebuilt_lineage_bytes")
        == raw_verification["expected_flattening_bytes"]
        and raw_verification["retained_flattening"] == flattened_lineage
        and raw_verification["lineage_cardinalities"] == cardinalities
        and raw_verification["all_exact"] is True
        and all(
            type(case) is dict
            ...
            and case.get("factor_lineage_manifest_ref") in identities
            and type(case.get("factor_certificate_lineage")) is tuple
            and case.get("factor_certificate_count")
            == len(case.get("factor_certificate_lineage", ()))
            and case.get("raw_lineage_verification")
            == verify_tensor_factor_lineage_case(
                case, census["complete_object_table"]
            )
            and case.get("raw_lineage_verification", {}).get("all_exact") is True
            and all(
...
def _tensor_lineage_entry_data(entry: tuple[Any, ...]) -> dict[str, Any]:
    _require_exact_tuple(entry, "tensor lineage entry")
    if len(entry) != 4:
        raise Refusal("tensor lineage entry has the wrong arity")
    address, input_ref, certificate_ref, pairing_key = entry
    _require_exact_tuple(address, "tensor lineage generator address")
    _require_exact(input_ref, str, "tensor lineage input reference")
    _require_exact(certificate_ref, str, "tensor lineage certificate reference")
    _require_exact_tuple(pairing_key, "tensor lineage pairing key")
    if (
        not address
        or any(type(index) is not int or index < 0 for index in address)
        or not _is_lower_hex(input_ref, 32)
        or not _is_lower_hex(certificate_ref, 32)
    ):
        raise Refusal("tensor lineage entry is not canonical")
    return {
        "factor_generator_address": address,
        "input_ref": input_ref,
...
def _tensor_lineage_entries_data(
    entries: tuple[tuple[Any, ...], ...]
) -> tuple[dict[str, Any], ...]:
    _require_exact_tuple(entries, "tensor lineage entries")
    return tuple(_tensor_lineage_entry_data(entry) for entry in entries)


def _tensor_manifest_binding_data(
    law: GammaLaw,
    left_source_presentation: SourcePresentation,
    right_source_presentation: SourcePresentation,
    left_witness: SourceGroupoidWitness,
    right_witness: SourceGroupoidWitness,
    left_factor_address: tuple[int, ...],
    right_factor_address: tuple[int, ...],
    retained_entries: tuple[tuple[Any, ...], ...],
) -> dict[str, Any]:
    """Bind decoded/re-encoded objects, not merely their references."""

    return {
        "law_canonical_bytes": canonical_bytes(law),
        "left_source_presentation_canonical_bytes": canonical_bytes(
            left_source_presentation
        ),
        "right_source_presentation_canonical_bytes": canonical_bytes(
            right_source_presentation
        ),
        "left_witness_canonical_bytes": canonical_bytes(left_witness),
        "right_witness_canonical_bytes": canonical_bytes(right_witness),
        "left_factor_address": left_factor_address,
        "right_factor_address": right_factor_address,
        "retained_entries": _tensor_lineage_entries_data(retained_entries),
    }


@dataclass(frozen=True, slots=True)
class TensorFactorLineageManifest:
    law_ref: str
    left_source_presentation_ref: str
    right_source_presentation_ref: str
    left_witness_ref: str
    right_witness_ref: str
    left_factor_address: tuple[int, ...]
    right_factor_address: tuple[int, ...]
    retained_entries: tuple[tuple[Any, ...], ...]
    ...
    def __post_init__(self) -> None:
        for value, label in (
            (self.law_ref, "manifest law reference"),
            (
                self.left_source_presentation_ref,
                "manifest left presentation reference",
            ),
            (
                self.right_source_presentation_ref,
                "manifest right presentation reference",
            ),
            (self.left_witness_ref, "manifest left witness reference"),
            (self.right_witness_ref, "manifest right witness reference"),
            (self.manifest_binding_sha256, "manifest binding hash"),
        ):
            _require_exact(value, str, label)
            if not _is_lower_hex(value, 32):
                raise Refusal(label + " is not a canonical SHA-256")
        if self.left_factor_address != (0,) or self.right_factor_address != (1,):
            raise Refusal("tensor manifest factor addresses are not the two roots")
        entries_data = _tensor_lineage_entries_data(self.retained_entries)
        expected_order = tuple(
            sorted(
                self.retained_entries,
                key=lambda entry: canonical_bytes((entry[0], entry[3])),
            )
        )
        if self.retained_entries != expected_order:
            raise Refusal("tensor manifest entries are not canonically sorted")
        keys = tuple((entry[0], entry[3]) for entry in self.retained_entries)
        if len(keys) != len({canonical_hash(key) for key in keys}):
            raise Refusal("tensor manifest repeats an address/pairing key")
        if len(entries_data) != len(self.retained_entries):
            raise Refusal("tensor manifest entry serialization is incomplete")

    def to_data(self) -> dict[str, Any]:
        return {
            "type": "TensorFactorLineageManifest",
            "law_ref": self.law_ref,
            "left_source_presentation_ref": self.left_source_presentation_ref,
            "right_source_presentation_ref": self.right_source_presentation_ref,
            "left_witness_ref": self.left_witness_ref,
            "right_witness_ref": self.right_witness_ref,
            "left_factor_address": self.left_factor_address,
            "right_factor_address": self.right_factor_address,
            "retained_entries": _tensor_lineage_entries_data(
                self.retained_entries
            ),
...
def _literal_tensor_factor_entries(
    law: GammaLaw,
    presentation: SourcePresentation,
    ...
    for generator_address in generator_leaf_addresses(presentation.arrow):
        for value in generator_certificate_inputs(
            ...
            rows.append(
                (
                    factor_address + generator_address,
                    _store_complete_bytes(store, value),
                    _store_complete_bytes(store, value.certificate),
                    certificate_pairing_key(value),
                )
            )
    return tuple(
        sorted(
            rows,
            key=lambda row: canonical_bytes((row[0], row[3])),
        )
    )


def produce_tensor_factor_lineage_manifest(
    law: GammaLaw,
    left_source_presentation: SourcePresentation,
    left_witness: SourceGroupoidWitness,
    right_source_presentation: SourcePresentation,
    right_witness: SourceGroupoidWitness,
    complete_object_store: dict[str, bytes],
) -> TensorFactorLineageManifest:
    """Literal producer.  The raw verifier below does not call this function."""

    for value, expected, label in (
        (law, GammaLaw, "tensor manifest law"),
        (
            left_source_presentation,
            SourcePresentation,
            "tensor manifest left presentation",
        ),
        (left_witness, SourceGroupoidWitness, "tensor manifest left witness"),
        (
            right_source_presentation,
            SourcePresentation,
            "tensor manifest right presentation",
        ),
        (right_witness, SourceGroupoidWitness, "tensor manifest right witness"),
    ):
        _require_exact(value, expected, label)
    if type(complete_object_store) is not dict or any(
        type(key) is not str or type(payload) is not bytes
        for key, payload in complete_object_store.items()
    ):
        raise Refusal("tensor manifest object store is foreign")
    if (
        left_witness.source != left_source_presentation
        or right_witness.source != right_source_presentation
    ):
        raise Refusal("tensor manifest witness has the wrong source presentation")
    if (
        presentation_from_arrow(
            relabel_arrow(left_source_presentation.arrow, left_witness)
        )
        != left_witness.target
        or presentation_from_arrow(
            relabel_arrow(right_source_presentation.arrow, right_witness)
        )
        != right_witness.target
    ):
        raise Refusal("tensor manifest witness has the wrong derived target")
    law_ref = _store_complete_bytes(complete_object_store, law)
    left_presentation_ref = _store_complete_bytes(
        complete_object_store, left_source_presentation
    )
    right_presentation_ref = _store_complete_bytes(
        complete_object_store, right_source_presentation
    )
    left_witness_ref = _store_complete_bytes(complete_object_store, left_witness)
    right_witness_ref = _store_complete_bytes(complete_object_store, right_witness)
    retained_entries = tuple(
        sorted(
            _literal_tensor_factor_entries(
                law, left_source_presentation, (0,), complete_object_store
            )
            + _literal_tensor_factor_entries(
                law, right_source_presentation, (1,), complete_object_store
            ),
            key=lambda entry: canonical_bytes((entry[0], entry[3])),
        )
    )
    binding = _tensor_manifest_binding_data(
        law,
        left_source_presentation,
        right_source_presentation,
        left_witness,
    ...
    return TensorFactorLineageManifest(
        law_ref,
        left_presentation_ref,
        right_presentation_ref,
        left_witness_ref,
        right_witness_ref,
        (0,),
        (1,),
...
def _decoded_frozen_sequence(value: Any, label: str) -> tuple[Any, ...]:
    if type(value) is not list:
        raise Refusal(f"decoded {label} is not a canonical array")
    return tuple(
        _decoded_frozen_sequence(item, label)
...
def _decode_tensor_factor_lineage_manifest_payload(
    payload: bytes,
) -> TensorFactorLineageManifest:
    try:
        raw = json.loads(payload.decode("ascii"))
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise Refusal("tensor manifest bytes are not canonical JSON") from error
    row = _decoded_mapping(
        raw,
        {
            "type",
            "law_ref",
            "left_source_presentation_ref",
            "right_source_presentation_ref",
            "left_witness_ref",
            "right_witness_ref",
            "left_factor_address",
            "right_factor_address",
            "retained_entries",
            "manifest_binding_sha256",
        },
        "TensorFactorLineageManifest",
    )
    if row["type"] != "TensorFactorLineageManifest":
        raise Refusal("decoded tensor manifest type tag is foreign")
    entries: list[tuple[Any, ...]] = []
    for raw_entry in _decoded_tuple(
        row["retained_entries"], "tensor manifest retained entries"
    ):
        entry = _decoded_mapping(
            raw_entry,
            {
                "factor_generator_address",
                "input_ref",
                "certificate_ref",
                "pairing_key",
            },
            "tensor manifest retained entry",
        )
        entries.append(
            (
                _decoded_frozen_sequence(
                    entry["factor_generator_address"],
                    "tensor manifest generator address",
                ),
                entry["input_ref"],
                entry["certificate_ref"],
                _decoded_frozen_sequence(
                    entry["pairing_key"], "tensor manifest pairing key"
                ),
            )
        )
    manifest = TensorFactorLineageManifest(
        row["law_ref"],
        row["left_source_presentation_ref"],
        row["right_source_presentation_ref"],
        row["left_witness_ref"],
        row["right_witness_ref"],
        _decoded_frozen_sequence(
            row["left_factor_address"], "tensor manifest left factor address"
        ),
        _decoded_frozen_sequence(
            row["right_factor_address"], "tensor manifest right factor address"
        ),
        tuple(entries),
        row["manifest_binding_sha256"],
    )
    if canonical_bytes(manifest) != payload:
        raise Refusal("decoded tensor manifest does not reproduce its bytes")
    return manifest


def _decode_lineage_certificate_input_payload(
    payload: bytes,
    law: GammaLaw,
    presentation: SourcePresentation,
    generator_ast_address: tuple[int, ...],
) -> CertificateActionInput:
    """Decode a nested input using the independently decoded presentation."""

    try:
        raw = json.loads(payload.decode("ascii"))
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise Refusal("lineage certificate input is not canonical JSON") from error
    required = {
        "type",
        "certificate",
        "law",
    ...
    row = _decoded_mapping(raw, required, "lineage CertificateActionInput")
    if row["type"] != "CertificateActionInput":
        raise Refusal("lineage certificate input type tag is foreign")
    source_configuration = _decode_configuration_data(row["source_configuration"])
    target_configuration = _decode_configuration_data(row["target_configuration"])
    leaf = _arrow_at_address(presentation.arrow, generator_ast_address)
    if leaf.kind != "GENERATOR":
        raise Refusal("lineage input address is not a generator leaf")
    certificate = build_bound_split_certificate(
        law, leaf, source_configuration, target_configuration
    )
    rebuilt = _assemble_certificate_action_input(
    ...
    if canonical_bytes(rebuilt) != payload:
        raise Refusal("lineage certificate input does not reproduce all bytes")
    return rebuilt


def _independently_rebuild_tensor_factor_entries(
    law: GammaLaw,
    presentation: SourcePresentation,
    factor_address: tuple[int, ...],
    payloads: Mapping[str, bytes],
) -> tuple[tuple[Any, ...], ...]:
    """Independent address/input route; never calls the literal producer."""

    rows: list[tuple[Any, ...]] = []
    derived_addresses = generator_leaf_addresses(presentation.arrow)
    for generator_address in derived_addresses:
        leaf = _arrow_at_address(presentation.arrow, generator_address)
        if leaf.kind != "GENERATOR":
            raise Refusal("independent tensor lineage found a nongenerator leaf")
        for value in generator_certificate_inputs(
            law, presentation, generator_address
        ):
            input_bytes = canonical_bytes(value)
            certificate_bytes = canonical_bytes(value.certificate)
            input_ref = sha256_bytes(input_bytes)
            certificate_ref = sha256_bytes(certificate_bytes)
            if (
                payloads.get(input_ref) != input_bytes
                or payloads.get(certificate_ref) != certificate_bytes
            ):
                raise Refusal("independent tensor lineage object is absent from store")
            rows.append(
                (
                    factor_address + generator_address,
                    input_ref,
    ...
    return tuple(
        sorted(rows, key=lambda entry: canonical_bytes((entry[0], entry[3])))
    )


def verify_tensor_factor_lineage_case(
    case: Mapping[str, Any], complete_store: Sequence[Mapping[str, Any]]
) -> dict[str, Any]:
    """Rebuild one manifest from authenticated bytes with no expected input."""

    label = case.get("label") if type(case) is dict else None
    manifest_ref = case.get("factor_lineage_manifest_ref") if type(case) is dict else None
    base = {
        "case_label": label,
        "manifest_ref": manifest_ref,
        "retained_entries": (),
        "expected_entries": (),
        "retained_lineage_bytes": canonical_bytes(()),
        "expected_lineage_bytes": canonical_bytes(()),
        "decoded_objects_exact": False,
        "case_binding_exact": False,
        "entry_bindings_exact": False,
        "manifest_binding_exact": False,
        "all_exact": False,
        "refusal": None,
    }
    try:
        if type(case) is not dict:
            raise Refusal("tensor lineage case is foreign")
        payloads = _complete_byte_store_payloads(complete_store)
        if not payloads or manifest_ref not in payloads:
            raise Refusal("tensor lineage manifest is absent from authenticated store")
        manifest = _decode_tensor_factor_lineage_manifest_payload(
            payloads[manifest_ref]
        )
        arrow_cache: dict[str, Arrow] = {}
        law_payload = payloads.get(manifest.law_ref)
        left_presentation_payload = payloads.get(
            manifest.left_source_presentation_ref
        )
        right_presentation_payload = payloads.get(
            manifest.right_source_presentation_ref
        )
        left_witness_payload = payloads.get(manifest.left_witness_ref)
        right_witness_payload = payloads.get(manifest.right_witness_ref)
        if any(
            payload is None
            for payload in (
                law_payload,
                left_presentation_payload,
                right_presentation_payload,
                left_witness_payload,
                right_witness_payload,
            )
        ):
            raise Refusal("tensor lineage manifest reference is absent")
        law = _decode_law_data(json.loads(law_payload.decode("ascii")))
        left_presentation = _decode_source_presentation_data(
            json.loads(left_presentation_payload.decode("ascii")), arrow_cache
        )
        right_presentation = _decode_source_presentation_data(
            json.loads(right_presentation_payload.decode("ascii")), arrow_cache
        )
        left_witness = _decode_witness_payload(left_witness_payload, arrow_cache)
        right_witness = _decode_witness_payload(right_witness_payload, arrow_cache)
        decoded_objects_exact = all(
            canonical_bytes(value) == payload
            for value, payload in (
                (law, law_payload),
                (left_presentation, left_presentation_payload),
                (right_presentation, right_presentation_payload),
                (left_witness, left_witness_payload),
                (right_witness, right_witness_payload),
            )
        )
        if (
            left_witness.source != left_presentation
            or right_witness.source != right_presentation
            or presentation_from_arrow(
                relabel_arrow(left_presentation.arrow, left_witness)
            )
            != left_witness.target
            or presentation_from_arrow(
                relabel_arrow(right_presentation.arrow, right_witness)
            )
            != right_witness.target
        ):
            raise Refusal("tensor lineage decoded witness is mistyped")
        tensor_source = presentation_from_arrow(
            tensor_arrow(left_presentation.arrow, right_presentation.arrow)
        )
        tensor_witness = make_groupoid_witness(
            tensor_source.arrow,
            left_witness.role_map + right_witness.role_map,
        ...
        law_row = case.get("law_row", {})
        left_source_root_ref = sha256_bytes(
            canonical_bytes(boundary_node_at(left_presentation, (), "SOURCE"))
        )
        right_source_root_ref = sha256_bytes(
            canonical_bytes(boundary_node_at(right_presentation, (), "SOURCE"))
        )
        left_target_root_ref = sha256_bytes(
            canonical_bytes(boundary_node_at(left_witness.target, (), "SOURCE"))
        )
        right_target_root_ref = sha256_bytes(
            canonical_bytes(boundary_node_at(right_witness.target, (), "SOURCE"))
        )
        tensor_target_root_ref = sha256_bytes(
            canonical_bytes(boundary_node_at(tensor_witness.target, (), "SOURCE"))
        )
        case_binding_exact = (
            case.get("left_source_root_ref") == left_source_root_ref
            and case.get("right_source_root_ref") == right_source_root_ref
            and case.get("left_target_root_ref") == left_target_root_ref
            and case.get("right_target_root_ref") == right_target_root_ref
            and case.get("tensor_target_root_ref") == tensor_target_root_ref
            and all(
                reference in payloads
                for reference in (
            ...
            and law_row.get("presentation_identities")
            == (
                presentation_identity(left_presentation),
                presentation_identity(right_presentation),
                presentation_identity(tensor_source),
            )
            and law_row.get("witness_identities")
            == (
                witness_identity(left_witness),
                witness_identity(right_witness),
                witness_identity(tensor_witness),
            )
        )
        expected_entries = tuple(
            sorted(
                _independently_rebuild_tensor_factor_entries(
                    law,
                    left_presentation,
                    manifest.left_factor_address,
                    payloads,
                )
                + _independently_rebuild_tensor_factor_entries(
                    law,
                    right_presentation,
                    manifest.right_factor_address,
                    payloads,
                ),
                key=lambda entry: canonical_bytes((entry[0], entry[3])),
            )
        )
        entry_bindings: list[bool] = []
        for entry in manifest.retained_entries:
            address, input_ref, certificate_ref, pairing_key = entry
            if address[0] == 0:
                ...
                raise Refusal("tensor lineage entry has an alien factor prefix")
            if input_ref not in payloads or certificate_ref not in payloads:
                raise Refusal("tensor lineage entry references an absent object")
            value = _decode_lineage_certificate_input_payload(
                payloads[input_ref], law, presentation, local_address
            )
            entry_bindings.append(
                payloads[certificate_ref] == canonical_bytes(value.certificate)
                and certificate_ref == sha256_bytes(payloads[certificate_ref])
                and pairing_key == certificate_pairing_key(value)
            )
        entry_bindings_exact = all(entry_bindings)
        binding_data = _tensor_manifest_binding_data(
            law,
            left_presentation,
            right_presentation,
            left_witness,
            ...
            manifest.left_factor_address,
            manifest.right_factor_address,
            manifest.retained_entries,
        )
        manifest_binding_exact = (
            manifest.manifest_binding_sha256 == canonical_hash(binding_data)
        )
        retained_data = _tensor_lineage_entries_data(manifest.retained_entries)
        expected_data = _tensor_lineage_entries_data(expected_entries)
        all_exact = (
            decoded_objects_exact
            and case_binding_exact
            and entry_bindings_exact
            and manifest_binding_exact
            and manifest.retained_entries == expected_entries
            and case.get("factor_certificate_lineage") == retained_data
            and case.get("factor_certificate_count") == len(retained_data)
        )
        return {
            **base,
            "retained_entries": retained_data,
            "expected_entries": expected_data,
            "retained_lineage_bytes": canonical_bytes(retained_data),
            "expected_lineage_bytes": canonical_bytes(expected_data),
            "decoded_objects_exact": decoded_objects_exact,
            "case_binding_exact": case_binding_exact,
            "entry_bindings_exact": entry_bindings_exact,
            "manifest_binding_exact": manifest_binding_exact,
            "all_exact": all_exact,
        }
    except (Refusal, IntegrityFailure, KeyError, TypeError, ValueError) as error:
        return {**base, "refusal": f"{type(error).__name__}:{error}"}


def _measure_tensor_lineage_raw_verification(
    cases: Sequence[Mapping[str, Any]],
    complete_store: Sequence[Mapping[str, Any]],
) -> dict[str, Any]:
    case_rows = tuple(
        verify_tensor_factor_lineage_case(case, complete_store) for case in cases
    )
    retained = tuple(
        entry for row in case_rows for entry in row["retained_entries"]
    )
    expected = tuple(
        entry for row in case_rows for entry in row["expected_entries"]
    )
    cardinalities = tuple(len(row["retained_entries"]) for row in case_rows)
    return {
        "case_rows": case_rows,
        "case_count": len(case_rows),
        "retained_flattening": retained,
        "expected_flattening": expected,
        "lineage_cardinalities": cardinalities,
        "retained_flattening_bytes": canonical_bytes(retained),
        "expected_flattening_bytes": canonical_bytes(expected),
        "all_exact": bool(case_rows)
        and all(row["all_exact"] for row in case_rows)
...
def _measure_tensor_configuration_case(
    ...
    manifest = produce_tensor_factor_lineage_manifest(
        law,
        left_witness.source,
        left_witness,
    ...
    manifest_ref = _store_complete_bytes(store, manifest)
    lineage = _tensor_lineage_entries_data(manifest.retained_entries)
    lineage_keys = tuple(
    ...
    return {
        ...
        "factor_lineage_manifest_ref": manifest_ref,
...
def measure_tensor_configuration_action(law: GammaLaw) -> dict[str, Any]:
    ...
    cases = tuple(
        {
            **case,
            "raw_lineage_verification": verify_tensor_factor_lineage_case(
                case, frozen_store
            ),
        }
        for case in raw_cases
    )
    raw_lineage_verification = _measure_tensor_lineage_raw_verification(
        cases, frozen_store
    )
    flattened_lineage = tuple(
    ...
    return {
        ...
        "factor_certificate_lineage": flattened_lineage,
        "lineage_cardinalities": lineage_cardinalities,
        "raw_lineage_verification": raw_lineage_verification,
        "independently_rebuilt_lineage_bytes": raw_lineage_verification[
            "expected_flattening_bytes"
        ],
        "all_exact": _complete_byte_store_exact(frozen_store)
        and len(cases) == 3
        and raw_lineage_verification["all_exact"]
        and raw_lineage_verification["retained_flattening"]
        == flattened_lineage
        and raw_lineage_verification["lineage_cardinalities"]
        == lineage_cardinalities
        and all(
            case["all_exact"]
            and case["raw_lineage_verification"]["all_exact"]
            ...
            and case["factor_lineage_manifest_ref"] in identities
            and all(
...
def groupoid_promotion_predicate(groupoid: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and tensor_configuration["raw_lineage_verification"]["all_exact"]
        and tensor_configuration["independently_rebuilt_lineage_bytes"]
        == tensor_configuration["raw_lineage_verification"][
            "expected_flattening_bytes"
        ]
        and all(
            ...
            and case["factor_lineage_manifest_ref"] in tensor_configuration_ids
...
@dataclass(frozen=True, slots=True)
class _GroupoidPromotionView:
    """Ephemeral verifier view retaining the complete in-memory preimage."""

    groupoid: Mapping[str, Any]
    exact: bool


def _build_groupoid_promotion_view(
    groupoid: Mapping[str, Any],
) -> _GroupoidPromotionView:
    return _GroupoidPromotionView(
        groupoid=groupoid,
        exact=groupoid_promotion_predicate(groupoid),
    )


def _groupoid_promotion_from_view(
    groupoid: Mapping[str, Any],
    view: _GroupoidPromotionView | None,
) -> bool:
    if view is None:
        return groupoid_promotion_predicate(groupoid)
    if type(view) is not _GroupoidPromotionView or view.groupoid is not groupoid:
        raise IntegrityFailure("groupoid promotion view is not bound to its preimage")
    return view.exact
...
def validate_public_exact_node(value: Any) -> None:
    ...
    elif type(value) is TensorFactorLineageManifest:
        _require_reconstruction_equal(
            value,
            TensorFactorLineageManifest(
                value.law_ref,
                value.left_source_presentation_ref,
                value.right_source_presentation_ref,
                value.left_witness_ref,
                value.right_witness_ref,
                value.left_factor_address,
                value.right_factor_address,
                value.retained_entries,
                value.manifest_binding_sha256,
            ),
            "tensor factor lineage manifest",
        )
...
def _tensor_manifest_objects_for_rebinding(
    manifest: TensorFactorLineageManifest,
    payloads: Mapping[str, bytes],
) -> tuple[
    GammaLaw,
    ...
    arrow_cache: dict[str, Arrow] = {}
    law = _decode_law_data(json.loads(payloads[manifest.law_ref].decode("ascii")))
    left_presentation = _decode_source_presentation_data(
        json.loads(
            payloads[manifest.left_source_presentation_ref].decode("ascii")
        ),
        arrow_cache,
    )
    right_presentation = _decode_source_presentation_data(
        json.loads(
            payloads[manifest.right_source_presentation_ref].decode("ascii")
        ),
        arrow_cache,
    )
    left_witness = _decode_witness_payload(
        payloads[manifest.left_witness_ref], arrow_cache
    )
    right_witness = _decode_witness_payload(
        payloads[manifest.right_witness_ref], arrow_cache
    )
    return law, left_presentation, right_presentation, left_witness, right_witness


def _manifest_payload_with_entries(
    manifest: TensorFactorLineageManifest,
    entries: tuple[tuple[Any, ...], ...],
    payloads: Mapping[str, bytes],
) -> bytes:
    law, left_presentation, right_presentation, left_witness, right_witness = (
        _tensor_manifest_objects_for_rebinding(manifest, payloads)
    )
    ordered = tuple(
        sorted(entries, key=lambda entry: canonical_bytes((entry[0], entry[3])))
    )
    binding = _tensor_manifest_binding_data(
        law,
        left_presentation,
    ...
    data["retained_entries"] = _tensor_lineage_entries_data(ordered)
    data["manifest_binding_sha256"] = canonical_hash(binding)
    return canonical_bytes(data)


def _replace_tensor_manifest(
    tensor_measurement: Mapping[str, Any],
    case_index: int,
    entries: tuple[tuple[Any, ...], ...],
) -> tuple[dict[str, Any], bytes, bytes]:
    if type(tensor_measurement) is not dict or type(case_index) is not int:
        raise Refusal("tensor manifest mutation input is foreign")
    store = tensor_measurement["complete_object_table"]
    payloads = _complete_byte_store_payloads(store)
    cases = list(tensor_measurement["cases"])
    case = dict(cases[case_index])
    old_ref = case["factor_lineage_manifest_ref"]
    old_payload = payloads[old_ref]
    manifest = _decode_tensor_factor_lineage_manifest_payload(old_payload)
    new_payload = _manifest_payload_with_entries(manifest, entries, payloads)
    new_ref = sha256_bytes(new_payload)
    replaced_store: dict[str, bytes] = {
        identity: payload
        for identity, payload in payloads.items()
        if identity != old_ref
    }
    prior = replaced_store.get(new_ref)
    if prior is not None and prior != new_payload:
        raise IntegrityFailure("mutated tensor manifest content address collides")
    replaced_store[new_ref] = new_payload
    frozen_store = _freeze_complete_byte_store(replaced_store)
    ordered_entries = tuple(
        sorted(entries, key=lambda entry: canonical_bytes((entry[0], entry[3])))
    )
    case["factor_lineage_manifest_ref"] = new_ref
    case["factor_certificate_lineage"] = _tensor_lineage_entries_data(
        ordered_entries
    )
    case["factor_certificate_count"] = len(ordered_entries)
    case["factor_keys_unique"] = len(ordered_entries) == len(
        {
            canonical_hash((entry[0], entry[3]))
            for entry in ordered_entries
        }
    )
    case["raw_lineage_verification"] = verify_tensor_factor_lineage_case(
        case, frozen_store
    )
    ...
    raw_verification = _measure_tensor_lineage_raw_verification(
        refreshed_cases, frozen_store
    )
    flattened = tuple(
        entry
        for value in refreshed_cases
        for entry in value["factor_certificate_lineage"]
    )
    cardinalities = tuple(
        len(value["factor_certificate_lineage"])
        for value in refreshed_cases
    )
    changed = dict(tensor_measurement)
    changed.update(
        {
            "complete_object_table": frozen_store,
            "complete_object_table_exact": _complete_byte_store_exact(
            ...
            "factor_certificate_lineage": flattened,
            "lineage_cardinalities": cardinalities,
            "raw_lineage_verification": raw_verification,
            "independently_rebuilt_lineage_bytes": raw_verification[
                "expected_flattening_bytes"
            ],
            "all_exact": raw_verification["all_exact"],
        }
    )
    return changed, old_payload, new_payload


def _tensor_lineage_unchanged_component_hashes(
    groupoid: Mapping[str, Any],
) -> dict[str, Any]:
    configuration_actions = tuple(
        row["configuration_action"]
        for row in groupoid["native_census"]["rows"]
    )
    return {
        "native_dependency_sha256": groupoid["native_dependency_sha256"],
        "configuration_action_row_hashes": tuple(
            canonical_hash(row) for row in configuration_actions
        ),
        "configuration_actions_sha256": canonical_hash(
...
def _tensor_lineage_dependency_slice(
    measurements: Mapping[str, Any],
    tensor_measurement: Mapping[str, Any],
    baseline_promotion_view: _GroupoidPromotionView | None = None,
    baseline_component_hashes: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    baseline_groupoid = measurements["groupoid"]
    changed_groupoid = dict(baseline_groupoid)
    changed_groupoid["tensor_configuration_action"] = tensor_measurement
    changed_measurements = dict(measurements)
    changed_measurements["groupoid"] = changed_groupoid
    tensor_exact = _tensor_configuration_action_exact(tensor_measurement)
    if baseline_promotion_view is None:
        promotion_view = _build_groupoid_promotion_view(changed_groupoid)
        groupoid_gate = _groupoid_promotion_from_view(
            changed_groupoid, promotion_view
        )
    else:
        if (
            type(baseline_promotion_view) is not _GroupoidPromotionView
            or baseline_promotion_view.groupoid is not baseline_groupoid
        ):
            raise IntegrityFailure(
                "lineage attack view is not bound to the baseline preimage"
            )
        non_tensor_preimage_exact = (
            set(changed_groupoid) == set(baseline_groupoid)
            and all(
                changed_groupoid[key] is baseline_groupoid[key]
                for key in baseline_groupoid
                if key != "tensor_configuration_action"
            )
        )
        groupoid_gate = (
            baseline_promotion_view.exact
            and non_tensor_preimage_exact
            and tensor_exact
        )
    component_hashes = (
        _tensor_lineage_unchanged_component_hashes(changed_groupoid)
        if baseline_component_hashes is None
        else baseline_component_hashes
    )
    if (
        type(component_hashes) is not dict
        or component_hashes["native_dependency_sha256"]
        != changed_groupoid["native_dependency_sha256"]
    ):
        raise IntegrityFailure("lineage component hashes are not preimage-bound")
    tensor_raw_sha256 = canonical_hash(
        tensor_measurement["raw_lineage_verification"]
    )
    tensor_rebuilt_sha256 = sha256_bytes(
        tensor_measurement["independently_rebuilt_lineage_bytes"]
    )
    outcome_index = (
        independent_outcome_index(changed_measurements)
        if tensor_exact
        else 1
    )
    law_root = law_identity(measurements["law"])
    action_lineage = {
        "primitive_roots": (law_root,),
        "root_count": 1,
        "all_same_root": groupoid_gate,
        "derivation_hash": canonical_hash(
            {
                "native_dependency_sha256": component_hashes[
                    "native_dependency_sha256"
                ],
                "configuration_action_rows": component_hashes[
                    "configuration_action_row_hashes"
                ],
                "complete_certificate_action": component_hashes[
                    "complete_certificate_action_sha256"
                ],
                "tensor_lineage_raw_verification": tensor_raw_sha256,
                "tensor_lineage_rebuilt_bytes_sha256": tensor_rebuilt_sha256,
    ...
    shadow_dag = {
        ...
        "inputs": (
            component_hashes["native_dependency_sha256"],
            component_hashes["configuration_actions_sha256"],
            component_hashes["complete_certificate_action_sha256"],
            tensor_raw_sha256,
            tensor_rebuilt_sha256,
            measurements["groupoid_promotion_static"]["ast_sha256"],
        ),
        "output_sha256": component_hashes["native_dependency_sha256"],
        "primitive_ids": (law_root,),
    }
    shadow_dag["arithmetic_type_dag_sha256"] = canonical_hash(
        shadow_dag
    )
    baseline_path_hashes = component_hashes.get("referent_path_hashes")
    if baseline_path_hashes is None:
        consumed_path_hashes = tuple(
            (path, canonical_hash(changed_measurements[path]))
            for path in CLAIM_INPUT_PATHS["referent"]
            if path in changed_measurements
        )
    else:
        consumed_path_hashes = tuple(
            (
                path,
                canonical_hash(changed_groupoid)
                if (
                    path == "groupoid"
                    and tensor_measurement
                    is not baseline_groupoid["tensor_configuration_action"]
                )
                else identity,
            )
            for path, identity in baseline_path_hashes
        )
    claim_input_dag = {
        "consumed_measurement_paths": tuple(
            path for path, _ in consumed_path_hashes
        ),
        "consumed_measurement_path_sha256": consumed_path_hashes,
        "content_addressed_dag_sha256": canonical_hash(
            consumed_path_hashes
        ),
    }
    tensor_measurement_evidence = {
        "measurement_sha256": canonical_hash(tensor_measurement),
        "complete_object_table_sha256": canonical_hash(
            tensor_measurement["complete_object_table"]
        ),
        "raw_lineage_verification_sha256": tensor_raw_sha256,
        "independently_rebuilt_lineage_bytes_sha256": tensor_rebuilt_sha256,
    }
    dependency = {
        "tensor_measurement": tensor_measurement_evidence,
        "groupoid_gate": groupoid_gate,
        "outcome_index": outcome_index,
        "action_lineage": action_lineage,
        "shadow_dag": shadow_dag,
        "claim_input_dag": claim_input_dag,
    }
    dependency["seal"] = {
        "measurement_sha256": canonical_hash(dependency["tensor_measurement"]),
        "gate_sha256": canonical_hash(dependency["groupoid_gate"]),
        "outcome_sha256": canonical_hash(dependency["outcome_index"]),
        "action_lineage_sha256": canonical_hash(dependency["action_lineage"]),
        "shadow_dag_sha256": canonical_hash(dependency["shadow_dag"]),
        "claim_input_dag_sha256": canonical_hash(dependency["claim_input_dag"]),
    }
    return dependency


def _typed_groupoid_v5_lineage_attack(
    identifier: str,
    measurements: Mapping[str, Any] | None = None,
    lineage_attack_context: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    if identifier not in V5_LINEAGE_ATTACKS:
        raise Refusal("unknown v5 tensor-lineage attack")
    if measurements is None:
        law = GammaLaw(Fraction(1, 2))
        tensor_measurement = measure_tensor_configuration_action(law)
        source_text = Path(__file__).read_text(encoding="utf-8")
        minimal_measurements: Mapping[str, Any] | None = None
    else:
        law = measurements["law"]
        tensor_measurement = measurements["groupoid"][
            "tensor_configuration_action"
        ]
        source_text = Path(__file__).read_text(encoding="utf-8")
        minimal_measurements = measurements
    clean_exact = _tensor_configuration_action_exact(tensor_measurement)
    old: Any
    new: Any
    changed_path = "typed_groupoid_v5.tensor_factor_lineage"
    killed_by = "INDEPENDENT-RAW-NO-DROP-TENSOR-LINEAGE-REBUILD"
    drop = "P13-REFERENT-PRESENTATION-ONLY"
    evidence: dict[str, Any]
    if identifier == "TENSOR-LINEAGE-PRODUCER-BYPASS":
        needle = (
            "    for generator_address in generator_leaf_addresses(presentation.arrow):\n"
            "        for value in generator_certificate_inputs(\n"
            "            law, presentation, generator_address\n"
        ...
        if source_text.count(needle) != 1:
            raise IntegrityFailure("tensor-lineage producer mutation anchor moved")
        mutant_source = source_text.replace(needle, replacement, 1)
        clean_scan = groupoid_promotion_source_scan(source_text)
        mutant_scan = groupoid_promotion_source_scan(mutant_source)
        old = {"producer_ast_sha256": clean_scan["ast_sha256"]}
        new = {"producer_ast_sha256": mutant_scan["ast_sha256"]}
        producer_ast_changed = (
            clean_scan["ast_sha256"] != mutant_scan["ast_sha256"]
        )
        passed = (
            clean_scan["all_exact"]
            and producer_ast_changed
            and (
                FROZEN_GROUPOID_PROMOTION_AST_SHA256 == "TO-BE-FROZEN"
                or not mutant_scan["all_exact"]
            )
        )
        evidence = {
            "clean_static_exact": clean_scan["all_exact"],
            "mutant_static_exact": mutant_scan["all_exact"],
            "producer_ast_changed": producer_ast_changed,
            "construction_sentinel_active": (
                FROZEN_GROUPOID_PROMOTION_AST_SHA256 == "TO-BE-FROZEN"
            ),
        }
        return _attack_record(
            old,
            new,
            "typed_groupoid_v5.lineage_producer_source",
            "POINT-FREE-TYPED-GROUPOID-ACTION",
            "PRODUCER-TO-PROMOTION-STATIC-BACKWARD-SLICE",
            passed,
            evidence,
            drop,
        )
    payloads = _complete_byte_store_payloads(
        tensor_measurement["complete_object_table"]
    )
    first_case = tensor_measurement["cases"][0]
    manifest = _decode_tensor_factor_lineage_manifest_payload(
        payloads[first_case["factor_lineage_manifest_ref"]]
    )
    entries = manifest.retained_entries
    if len(entries) < 2:
        raise IntegrityFailure("tensor-lineage attack requires two entries")
    changed_entries: tuple[tuple[Any, ...], ...]
    if identifier == "TENSOR-FACTOR-LINEAGE-DROP-ALL":
        changed_entries = ()
    elif identifier == "TENSOR-FACTOR-LINEAGE-DROP-ONE":
        changed_entries = entries[1:]
    elif identifier == "TENSOR-FACTOR-LINEAGE-DUPLICATE":
        changed_entries = entries + (entries[0],)
    elif identifier == "TENSOR-FACTOR-LINEAGE-SAME-COUNT-SWAP":
        first_index = 0
        second_index = next(
            index
            for index, entry in enumerate(entries)
            if entry[0] != entries[first_index][0]
        )
        first, second = entries[first_index], entries[second_index]
        changed_rows = list(entries)
        changed_rows[first_index] = (
            first[0], second[1], second[2], second[3]
        )
        changed_rows[second_index] = (
            second[0], first[1], first[2], first[3]
        )
        changed_entries = tuple(changed_rows)
    elif identifier == "TENSOR-FACTOR-LINEAGE-FOREIGN-INPUT":
        foreign_case = tensor_measurement["cases"][1]
        foreign_manifest = _decode_tensor_factor_lineage_manifest_payload(
            payloads[foreign_case["factor_lineage_manifest_ref"]]
        )
        foreign = foreign_manifest.retained_entries[0]
        changed_entries = (
            (entries[0][0], foreign[1], foreign[2], foreign[3]),
        ) + entries[1:]
    elif identifier == "TENSOR-FACTOR-CERTIFICATE-REF-SEVER":
        first, second = entries[0], entries[1]
        changed_entries = (
            (first[0], first[1], second[2], first[3]),
            (second[0], second[1], first[2], second[3]),
        ) + entries[2:]
    elif identifier == "TENSOR-FACTOR-PAIRING-KEY-SEVER":
        changed_entries = (
            (entries[0][0], entries[0][1], entries[0][2], entries[1][3]),
        ) + entries[1:]
    else:
        raise Refusal("unknown v5 tensor-lineage data attack")
    changed_tensor, old_payload, new_payload = _replace_tensor_manifest(
        tensor_measurement, 0, changed_entries
    )
    changed_exact = _tensor_configuration_action_exact(changed_tensor)
    passed = clean_exact and not changed_exact
    old = old_payload
    new = new_payload
    evidence = {
        "clean_tensor_exact": clean_exact,
        "mutant_tensor_exact": changed_exact,
        "clean_lineage_count": len(entries),
        "mutant_lineage_count": len(changed_entries),
        "complete_store_exact": changed_tensor["complete_object_table_exact"],
        "raw_verifier_exact": changed_tensor["raw_lineage_verification"][
            "all_exact"
        ],
    }
    if minimal_measurements is not None:
        clean_dependency = (
            lineage_attack_context["clean_dependency"]
            if lineage_attack_context is not None
            else _tensor_lineage_dependency_slice(
                minimal_measurements,
                tensor_measurement,
                lineage_attack_context.get("promotion_view")
                if lineage_attack_context is not None
                else None,
                lineage_attack_context.get("component_hashes")
                if lineage_attack_context is not None
                else None,
            )
        )
        changed_dependency = _tensor_lineage_dependency_slice(
            minimal_measurements,
            changed_tensor,
            lineage_attack_context.get("promotion_view")
            if lineage_attack_context is not None
        ...
        moved = tuple(
            ...
            if canonical_bytes(clean_dependency[key])
            != canonical_bytes(changed_dependency[key])
        )
        evidence.update(
            {
                "clean_dependency_sha256": canonical_hash(clean_dependency),
                "mutant_dependency_sha256": canonical_hash(changed_dependency),
                "moved_dependency_fields": moved,
                "all_dependency_fields_moved": len(moved) == 7,
                "clean_outcome_index": clean_dependency["outcome_index"],
                "mutant_outcome_index": changed_dependency["outcome_index"],
            }
        )
        passed = passed and len(moved) == 7
    return _attack_record(
        old,
        new,
        changed_path,
...
def recompute_shadow_predicates(
    measurements: Mapping[str, Any],
    promotion_view: _GroupoidPromotionView | None = None,
) -> dict[str, bool]:
    ...
    shadow_recomputation = {
        ...
        "typed_groupoid_action": _groupoid_promotion_from_view(
            measurements["groupoid"], promotion_view
        ),
    }
...
def build_result_neutral_regression_wall(
    ...
    predicates = {
        ...
        "source_groupoid_doctrine_exact": _groupoid_promotion_from_view(
            measurements["groupoid"], promotion_view
        ),
...
def _lineage_gate(
    ...
    rows["typed_groupoid_action"] = {
        ...
        "root_count": 1,
        "all_same_root": _groupoid_promotion_from_view(
            groupoid, promotion_view
        ),
        "derivation_hash": canonical_hash(
            {
                ...
                "tensor_lineage_raw_verification": canonical_hash(
                    groupoid["tensor_configuration_action"][
                        "raw_lineage_verification"
                    ]
                ),
                "tensor_lineage_rebuilt_bytes_sha256": sha256_bytes(
                    groupoid["tensor_configuration_action"][
                        "independently_rebuilt_lineage_bytes"
                    ]
                ),
                "promotion_backward_slice_ast_sha256": measurements[
                    "groupoid_promotion_static"
...
def build_shadow_lineages(
    ...
    blind = measurements["blind_family"]
    support = measurements["support_change"]
    shadow_recomputation = recompute_shadow_predicates(
        measurements, promotion_view
    )
    rows: dict[str, Any] = {
        "primitive": {
            "operation": "ONE-ROOT",
...
def groupoid_promotion_source_scan(source: str) -> dict[str, Any]:
    ...
    backward_slice_names = (
        ...
        "measure_tensor_configuration_action",
        "_measure_tensor_configuration_case",
        "produce_tensor_factor_lineage_manifest",
        "_literal_tensor_factor_entries",
        "_tensor_lineage_entry_data",
        "_tensor_lineage_entries_data",
        "_tensor_manifest_binding_data",
        "_decoded_frozen_sequence",
        "_decode_tensor_factor_lineage_manifest_payload",
        "_decode_lineage_certificate_input_payload",
        "_independently_rebuild_tensor_factor_entries",
        "verify_tensor_factor_lineage_case",
        "_measure_tensor_lineage_raw_verification",
        ...
        "_generator_map",
        "_build_groupoid_promotion_view",
        "_groupoid_promotion_from_view",
    )
    ...
    lineage_producer_signature_exact = exact_signature(
        "produce_tensor_factor_lineage_manifest",
        (
            "law",
            "left_source_presentation",
            ...
            "right_source_presentation",
            "right_witness",
            "complete_object_store",
        ),
    )
    lineage_verifier_signature_exact = exact_signature(
        "verify_tensor_factor_lineage_case", ("case", "complete_store")
    )
    ...
    lineage_producer_source = ast.get_source_segment(
        source, functions.get("produce_tensor_factor_lineage_manifest")
    ) or ""
    lineage_verifier_source = ast.get_source_segment(
        source, functions.get("verify_tensor_factor_lineage_case")
    ) or ""
    ...
    lineage_closed_dataflow_exact = (
        lineage_producer_signature_exact
        and lineage_verifier_signature_exact
        and "_literal_tensor_factor_entries" in lineage_producer_source
        and "_independently_rebuild_tensor_factor_entries"
        in lineage_verifier_source
        and "produce_tensor_factor_lineage_manifest"
        not in lineage_verifier_source
        and all(
            token not in lineage_verifier_source
            for token in (
                "global_registry",
                "history_cache",
    ...
    return {
        ...
        "lineage_producer_signature_exact": lineage_producer_signature_exact,
        "lineage_verifier_signature_exact": lineage_verifier_signature_exact,
        "lineage_closed_dataflow_exact": lineage_closed_dataflow_exact,
        ...
        and lineage_producer_signature_exact
        and lineage_verifier_signature_exact
        and lineage_closed_dataflow_exact
...
def build_gate_table(
    ...
    return {
        ...
        and _groupoid_promotion_from_view(
            measurements["groupoid"], promotion_view
        ),
...
def independent_outcome_index(measurements: Mapping[str, Any]) -> int:
    ...
    predicates = (
        ...
        and _tensor_configuration_action_exact(
            measurements["groupoid"]["tensor_configuration_action"]
        ),
...
def measure_scientific_core(source_text: str) -> dict[str, Any]:
    ...
    promotion_view = _build_groupoid_promotion_view(measurements["groupoid"])
    ...
    measurements["referent_census"] = {
        ...
        "groupoid_transport_exact": _groupoid_promotion_from_view(
            measurements["groupoid"], promotion_view
        )
        and measurements["groupoid"]["split_certificate_covariance"]["all_exact"]
    ...
    measurements["regression_wall"] = build_result_neutral_regression_wall(
        measurements, source_text, promotion_view
    )
    measurements["lineage"] = _lineage_gate(
        law, measurements, promotion_view
    )
    measurements["shadow_lineages"] = build_shadow_lineages(
        measurements, promotion_view
    )
    measurements["exposed_anchor_assertions"] = assert_exposed_controls(measurements)
    measurements["gates"] = build_gate_table(measurements, promotion_view)
...
V5_LINEAGE_ATTACKS = (
    "TENSOR-FACTOR-LINEAGE-DROP-ALL",
    "TENSOR-FACTOR-LINEAGE-DROP-ONE",
    "TENSOR-FACTOR-LINEAGE-DUPLICATE",
    "TENSOR-FACTOR-LINEAGE-SAME-COUNT-SWAP",
    "TENSOR-FACTOR-LINEAGE-FOREIGN-INPUT",
    "TENSOR-FACTOR-CERTIFICATE-REF-SEVER",
    "TENSOR-FACTOR-PAIRING-KEY-SEVER",
    "TENSOR-LINEAGE-PRODUCER-BYPASS",
)
GROUPID_ATTACKS = (
    "RELABEL-RAW-NAME",
    "RELABEL-ORIENTATION",
) + TYPED_GROUPOID_ATTACKS + V3_TYPED_GROUPOID_ATTACKS + V4_TYPED_GROUPOID_ATTACKS + V5_LINEAGE_ATTACKS
...
def run_attack(
    ...
    if identifier in GROUPID_ATTACKS:
        if identifier in V5_LINEAGE_ATTACKS:
            return _typed_groupoid_v5_lineage_attack(
                identifier, measurements, lineage_attack_context
            )
...
def run_all_attacks(measurements: Mapping[str, Any]) -> dict[str, Any]:
    ...
    lineage_promotion_view = _build_groupoid_promotion_view(
        measurements["groupoid"]
    )
    lineage_component_hashes = _tensor_lineage_unchanged_component_hashes(
        measurements["groupoid"]
    )
    lineage_component_hashes["referent_path_hashes"] = tuple(
        (path, canonical_hash(measurements[path]))
        for path in CLAIM_INPUT_PATHS["referent"]
        if path in measurements
    )
    lineage_attack_context = {
        "promotion_view": lineage_promotion_view,
        "component_hashes": lineage_component_hashes,
        "clean_dependency": _tensor_lineage_dependency_slice(
            measurements,
            measurements["groupoid"]["tensor_configuration_action"],
            lineage_promotion_view,
            lineage_component_hashes,
        )
    }
    rows = {
        identifier: run_attack(
            ...
            lineage_attack_context if identifier in V5_LINEAGE_ATTACKS else None,
        )
...
SOURCE_FREEZE_TASK_PATHS = (
    "v16/code/p13_gamma_exact_v5.py",
    "v16/code/p13_gamma_exact_v5_source_mutations.json",
    "v16/code/p13_gamma_exact_v5_source_mutation_replay.py",
    "v16/code/p13_gamma_exact_v5_source_mutation_receipt.json",
    "v16/note-paper13-typed-groupoid-source-freeze-v5.md",
)
PUBLICATION_PATHS = (
    "v16/code/p13_gamma_fresh_cases_v5.json",
    "v16/code/p13_gamma_output_v5.txt",
    "v16/code/p13_gamma_receipt_v5.json",
)
    ...
    reads.append(
        {
            ...
            "path": "v16/code/p13_gamma_exact_v5.py",
...
def _public_language_selfcheck(measurements: Mapping[str, Any]) -> dict[str, Any]:
    ...
    tensor_measurement = measurements["groupoid"]["tensor_configuration_action"]
    tensor_payloads = _complete_byte_store_payloads(
        tensor_measurement["complete_object_table"]
    )
    tensor_manifest_ref = tensor_measurement["cases"][0][
        "factor_lineage_manifest_ref"
    ]
    tensor_manifest = _decode_tensor_factor_lineage_manifest_payload(
        tensor_payloads[tensor_manifest_ref]
    )
    nodes = (
        formula_constant(False),
...
def _read_ledger_selfcheck() -> dict[str, Any]:
    ...
    official_rows = (
        {
            "open_index": 1,
            "path": "v16/code/p13_gamma_fresh_cases_v5.json",
    ...
    return {
        ...
        == "v16/code/p13_gamma_fresh_cases_v5.json"
        and official_rows[1]["path"] == "v16/code/p13_gamma_exact_v5.py",
...
def run_selftest() -> dict[str, Any]:
    ...
    groupoid_selftest_summary = {
        ...
        "tensor_lineage_cardinalities": groupoid[
            "tensor_configuration_action"
        ]["lineage_cardinalities"],
        "tensor_raw_lineage_verification_sha256": canonical_hash(
            groupoid["tensor_configuration_action"][
                "raw_lineage_verification"
            ]
        ),
        "tensor_independently_rebuilt_lineage_sha256": sha256_bytes(
            groupoid["tensor_configuration_action"][
                "independently_rebuilt_lineage_bytes"
            ]
        ),
    ...
    checks = (
        ...
        _selftest_check(
            "L1-L8-V5-TENSOR-LINEAGE-ATTACKS-KILLED",
            all(attacks["rows"][name]["pass"] for name in V5_LINEAGE_ATTACKS)
            and tuple(
                name for name in MUTANT_IDS if name in V5_LINEAGE_ATTACKS
            )
            == V5_LINEAGE_ATTACKS,
            {
                "names": V5_LINEAGE_ATTACKS,
                "rows": {
                    name: attacks["rows"][name]
                    for name in V5_LINEAGE_ATTACKS
                },
            },
        ),
        _selftest_check(
...
CLAIM_FALSIFIERS = {
    ...
    "referent": (
        ...
        "OLD-CHILD-REUSE",
        "CONTEXTUAL-BOOLEAN-ALIAS",
    ) + V3_TYPED_GROUPOID_ATTACKS + V4_TYPED_GROUPOID_ATTACKS + V5_LINEAGE_ATTACKS,
...
def build_receipt(
    ...
    artifacts = {
        ...
        "historical_old_source": HISTORICAL_OLD_SOURCE_PROVENANCE,
        "rejected_v4_source_cycle": {
            "source_sha256": REJECTED_V4_SOURCE_SHA256,
            "freeze_sha256": REJECTED_V4_FREEZE_SHA256,
            "operator_report_sha256": V4_OPERATOR_SOURCE_REPORT_SHA256,
            "records_report_sha256": V4_RECORDS_SOURCE_REPORT_SHA256,
...
def generate_fresh_mode(parsed: Mapping[str, str]) -> dict[str, Any]:
    ...
    _validate_publication_role(
        destination, "v16/code/p13_gamma_fresh_cases_v5.json"
    )
...
def run_official_mode(parsed: Mapping[str, str]) -> dict[str, Any]:
    ...
    _validate_publication_role(
        fresh_path, "v16/code/p13_gamma_fresh_cases_v5.json"
    )
    _validate_publication_role(output_path, "v16/code/p13_gamma_output_v5.txt")
    _validate_publication_role(receipt_path, "v16/code/p13_gamma_receipt_v5.json")
