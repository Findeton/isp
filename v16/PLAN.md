...
2. **Arity and curvature.**  test a viable two-actor occurrence, idle-spectator
   ...
   the primitive process arity.
3. **Representational recovery.**  determine exactly what is and is not fixed
   by a frozen-geometry transfer law: Hamiltonian branch/clock ambiguity,
   absence of a canonical global Hamiltonian on changing geometry, and the
   dependence of normal-mode/particle candidates on the still-free law.
4. **Selection and consequences.**  build rival laws on the same ontology.
   If more than one survives with different predictions, stamp the architecture
   underdetermined and classify every proposed QFT/GR, EPR, affine,
   cosmological, scale, and species consequence as forced, conditional,
   merely permitted, or refused.
...
3. run one official solve, render every claim from the sealed receipt, commit
   ...
   serialization; the second attempt is committed as-is at #23 and verified
...
The data-only recurring-context fixture and verdict-neutral scorer are frozen
at #62 before first execution. They declare two incompatible two-phase
contexts, one rich training context, one held-out rich context, an asymmetric
calibration control, four recurrence dictionaries, calibrated port and flag
data, a nonnormal pair, extreme-point maps, and fixed-Bob safety controls.
...
- A continuum/GR recovery unit, only after a discrete constraint algebra or
  equivalent covariance criterion is stated independently of Einstein's
  equations.
- An excitation/species unit, only after the vacuum and transport law are
...
- Paper 23a v2 (Unit B-v2, user-authorized): **TERMINAL ACCEPT-WITH-SCOPE
  at #321** (`eca8e167...`, post-repair), **corrected forward-only at #322:
  unital withdrawn** — the user-ordered recheck of Prop I against Paper 13D
  §§5.2/7.1/10.2 confirmed that fusion with an empty component traverses a
  fusion vertex, so its trace shape (Def 2.3) differs from the unfused
  complex and Prop B separates it; Exec_D declares no fusion-unit equation.
  Corrected tag
  ...
  verified byte-exact #321. Result (as corrected at #322): the certified
  Γ_D fixtures carry a commutative graded nonassociative magma WITHOUT
  unit of trace-sensitive sector classes; finite closure fails by
  carrier monotonicity; duals absent; FP fusion-bootstrap route CLOSED
  for the present Γ_D, fixture-scoped. Salvage from v1 (#314) fully used.
