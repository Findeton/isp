...
- **Live state pointer:** read both `v12/LOG.md` and `v13/LOG.md` from their
  ...
  binding order is: W6/Paper 2 record descent → RQ0 regional relativistic
...
- **§14 addendum (the gate-to-disk seal; v14 #119):** a verdict or published row is sealed (digested) at the moment its gate passes; artifacts are written from the sealed object; the terminal integrity check compares disk bytes against the gate-time seal.  A re-derivation from disk is not an integrity check — it confirms corruption.  Demonstrated at R4b (a NO-MOTION head delivered at exit 0 in both artifacts while the receipt's own rows said MOVES=57: '#234 satisfied at gate time, vacated at delivery time') and in the weld-2 INJ05/10/11 class.

- **§14 addendum (text gates match text as written; v14 #125):** any gate matching prose against a needle whitespace-normalises both sides, uses #62-anchored needles with a length floor, and includes the corpus's canonical short fragments.  Demonstrated at U4: the L-1 prohibition gate passed the banned 96-char sentence when inserted line-wrapped in house style (147/147, exit 0, artifacts written) — found independently by two seats; the two-line normalisation repair verified byte-identical on the pristine paper.

- **#119 addendum (the seal manifest is total; v14 #148):** every published receipt key is sealed at gate time or explicitly declared unsealed in the manifest; seals taken at the final gate are non-compliant.  Demonstrated at R5: a twelve-wide seam that was exactly the complement of a working seal (18 of 38 keys unsealed; three late seals), with the seal mechanism itself ruled sound — the prior playbook dead six-for-six.
