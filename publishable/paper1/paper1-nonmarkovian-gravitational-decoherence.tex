...
S(E_G,U)$, (ii) additivity of independent cost channels $S(E_1+E_2,T)=S(E_1,T)\,S(E_2,T)$, and
(iii) dependence only on the dimensionless action $a$, then the Cauchy functional equation forces
\begin{equation}
...
with no decay---a separate entry in the Sec.~\ref{sec:disc} classification, already bounded by
optomechanics~\cite{R24}). For a fixed preparation the dynamics is an ordinary \emph{linear} dephasing
...
A non-Markovian \emph{environment} produces the same quadratic onset~\cite{R16}, so curvature alone
...
essentially \emph{blind to bulk mass density} at fixed external size~\cite{R16}. Comparing same-size,
...
rate. (Both axes are plain, non-relativistic ISP: indivisibility supplies the onset, and the
...
alternative appears instead as a deterministic frequency \emph{shift} with no decay, already bounded
by optomechanics~\cite{R24}. A result in the (Gaussian onset) $\times$ (gravitational
...
leading correction is set by the excess kurtosis of $p(\delta E)$ ($\approx8\%$ of the onset exponent
for a bimodal $\delta E$, growing as $p$ becomes more discrete), and it occupies the \emph{same}
...
$\tau_c\gtrsim\tau_G$ is \emph{forced} rather than chosen. First, \textbf{records are information-gated}:
...
indivisibility motivates the finite memory---with $\tau_c$ pinned to the gravitational scale three
ways: by the Sec.~\ref{subsec:micro} fixed point, by information-gating, and by the demand for an
observable DP-scale effect. The DP-Markovian limit is the unphysical $\sigma_E\to\infty$ corner. The
...
this worst case is suppressed by $10^{-36}$, and the emission margin itself reads out the division
microstructure of Sec.~\ref{subsec:micro}. In this sense the finite memory acts as a \emph{temporal}
...
\bibitem{R13} E. Nelson, \emph{Derivation of the Schr\"odinger equation from Newtonian mechanics}, Phys. Rev. \textbf{150}, 1079 (1966).
