...
CLAIM (the split, honesty-graded):
  [CONSTRAINED]  The almost-quantum ENVELOPE is FORCED FROM INSIDE the records:
                 ...
                 (level-(1+AB) 0.2514709 > level-2 0.2509397 > Q-bound 0.2508754,
...
#   level-(1+AB) 0.2514709 > level-2 0.2509397 > Q-bound 0.2508754 (Pal-Vertesi 2010,
...
def cg_i3322_bound(level):
    """Maximize the Collins-Gisin I3322 over the NPA moment matrix.
       ...
       level=2      : ADDS same-party pairs PA[x]PA[x'], PB[y]PB[y'] in BOTH orders
                      (x != x' -- projectors of different settings do NOT commute, so
                      A{x}A{x'} and A{x'}A{x} are distinct words; 28 words total) ->
                      the genuine full NPA level 2, an UPPER bound on Q.  (The earlier
                      x<x'-only 22-word list is a PARTIAL level 2 -- it gave 0.2513864,
                      an intermediate relaxation between 1+AB and true level 2.)"""
    nA, nB = 3, 3
...
check("ENVELOPE: CG-I3322 strict-outer NESTING 1+AB > level-2 > Q "
      "(0.2514709 > 0.2509397 > 0.2508754; level-2 = full 28-word both-orders list, "
      "consistent with independent full-level-2 computations, 0.2509397...) => 1+AB is a genuine Q-tilde-not-Q relaxation",
...
# SHARPNESS scan (facet, not just far-exterior): just-INSIDE feasible, just-OUTSIDE
# infeasible along two directions -- this is what makes "the facet flips at sum^2=8"
# a verified statement rather than a 3-point bracket.
f_inside_sym  = chsh_pair_feasible(1.9, 1.9)                       # 7.22 < 8
f_out_sym     = chsh_pair_feasible(2.05, 2.05)                     # 8.405 > 8
f_out_asym    = chsh_pair_feasible(2*float(mp.sqrt(2)), 0.30)      # 8.09 > 8
...
line("SHARPNESS just-inside (1.9,1.9) [sum^2=7.22]", f"feasible={f_inside_sym}")
line("SHARPNESS just-outside (2.05,2.05) [sum^2=8.41]", f"feasible={f_out_sym}")
line("SHARPNESS just-outside (2sqrt2,0.30) [sum^2=8.09]", f"feasible={f_out_asym}")
check("ENVELOPE: facet SHARPNESS -- just-inside (7.22) feasible; just-outside "
      "(8.41 symmetric, 8.09 asymmetric) both INFEASIBLE => the flip is AT sum^2=8",
      f_inside_sym and (not f_out_sym) and (not f_out_asym),
      "the facet is verified sharp, not merely bracketed by 3 points", solver_tol=True)
