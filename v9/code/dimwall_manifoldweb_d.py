#!/usr/bin/env python3
"""
dimwall_manifoldweb_d.py — v9 round 45d: the monopole-free ballistic
relation (pin: the Round-45c/45d section of note-3p1-manifoldweb,
ab36005, strictly before this ran). Builder = 45c verbatim (dipole
kernel, full churn); THE RELATION changes: clocks tau*b + (chi_k -
chi_mean) — time = birth order, space = the content dipole; tau = c *
s_D/s_b (matched variance), c in {0.5, 1, 2}. Wiring reproduces two
45c rows under the OLD relation first.
...
def ballistic_rel(chiV, idx, c_dial):
    """The 45d relation on a window subsample: clocks tau*b +
    (chi_k - chi_mean); weak dominance in all K + b tie-break."""
    chi = chiV[idx]
    D = chi - chi.mean(axis=1, keepdims=True)     # monopole removed
    b = idx.astype(float)
    s_D = float(D.std())
    s_b = float(b.std())
    tau = c_dial * (s_D / max(s_b, 1e-12))
    ell = tau * b[:, None] + D                     # (NW, K) clocks
    rel = b[:, None] < b[None, :]                  # b tie-break/strict
    K = D.shape[1]
    for k in range(K):
        rel &= ell[:, None, k] <= ell[None, :, k]
    np.fill_diagonal(rel, False)
    coords = np.column_stack([b, D])
    return rel, coords
...
# ---- Gw (wiring): two round-45c rows under the OLD relation ----
R45C = {(4, 0.0): 3.156, (4, 0.75): 2.049}
okw = True
for (K, alpha), expect in R45C.items():
    Fs = []
    ...
    got = round(float(np.nanmean(Fs)), 3)
    okw &= got == expect
    print(f"      wiring K={K} a={alpha}: F_iso {got} vs 45c's {expect}")
check("Gw (wiring): 45c rows reproduced under the old relation", okw)
if not okw:
    print("WIRING REFUSAL."); print("FAILURES: 1"); raise SystemExit(1)

# ---- Gm1/Gm2: the grid (45d: the ballistic relation) ----
...
for K in (8, 16, 24):
    for (alpha, CDIAL) in ((0.75, 0.5), (0.75, 1.0), (0.75, 2.0),
                           (0.9, 1.0)):
        Fs, F2s, eigs, prs, frs, wfrs, refusals = [], [], [], [], [], [], 0
        Fembs = []
        rels = []
        for sd in SEEDS:
            _, chiV, _, _, rng = build_manifold(sd, K, alpha,
                                                churn="full",
                                                kernel="dipole")
            start = (2048 - 1024) // 2
            widx = np.sort(rng.choice(np.arange(start, start + 1024), 256,
                                      replace=False))
            wrel, coords = ballistic_rel(chiV, widx, CDIAL)
            # full-web relation for the dimension window + witnesses
            allidx = np.arange(2048)
            rel, _ = ballistic_rel(chiV, allidx, CDIAL)
        ...
        rows[(K, alpha, CDIAL)] = dict(F=float(np.nanmean(Fs)),
                                F2=float(np.nanmean(F2s)),
                                Femb=float(np.nanmean(Fembs)),
                                refusals=refusals, dmm=dmw,
                                pairs=int(np.mean(prs)), rels=rels)
        print(f"      K={K:2d} a={alpha:4.2f} c={CDIAL:3.1f}: F_iso = {np.nanmean(Fs):6.3f} "
    ...
    for (rel, rng) in rows[key]["rels"]:
        ...
        if K >= 16:
            ...
            if r5 and verify_sk(rel, r5[0], r5[1], 5):
                h5 += 1
    witness[key] = (h4, h5)
    print(f"      {key}: S4 {h4}/5" + (f", S5 {h5}/5" if K >= 16 else ""),
          flush=True)
check("Gm2: the ladder searched per pin", True)

# Gm3: the verdict
cands = [k for k, r in rows.items()
         if np.isfinite(r["F"]) and r["F"] <= 1.1 * MAXM4
         and r["refusals"] >= 8 and r["dmm"] >= 3.7
         and witness.get(k, (0, 0))[0] >= 3]
lowish = [k for k, r in rows.items()
