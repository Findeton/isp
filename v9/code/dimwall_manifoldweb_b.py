#!/usr/bin/env python3
"""
dimwall_manifoldweb_b.py — v9 round 45b: the full-vector-churn fix
(pin: the Round-45 amendment of note-3p1-manifoldweb, fca1493,
strictly before this ran). Derived from dimwall_manifoldweb.py with a
churn parameter; wiring reproduces two round-45 rows under the
per-clock flag before the full-vector grid runs.
...
# ---- the grown builder ----
def build_manifold(sd, K, alpha, churn="perclock", N=2048, M=32, L=16,
                   D=1024, NW=256):
    """Direction-valued deposits; churn='perclock' (round 45) or
    'full' (45b: one victim slot's whole K-vector reset per firing)."""
...
# ---- Gw (wiring): two round-45 rows under the per-clock flag ----
R45 = {(4, 0.0): 1.439, (4, 0.75): 1.821}
okw = True
for (K, alpha), expect in R45.items():
    Fs = []
    ...
    got = round(float(np.nanmean(Fs)), 3)
    okw &= got == expect
    print(f"      wiring K={K} a={alpha}: F_iso {got} vs 45's {expect}")
check("Gw (wiring): round-45 rows reproduced under the per-clock flag", okw)
if not okw:
    print("WIRING REFUSAL."); print("FAILURES: 1"); raise SystemExit(1)

# ---- Gm1/Gm2: the grid (FULL-VECTOR churn) ----
