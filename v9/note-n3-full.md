...
- **MINOR-5 (attribution, conceded):** under the Tier-A design the latent field is drawn *after and independently of* the web, so the web's entire contribution to closure is the **chain roster** (Mc — deterministically 64 on the corner arm) plus the confound variables; a web-free replication reproduces the closure bands at matched Mc (review-run, 200 draws). **The corner arm's genuine content is the HARVEST resolution** — slot-worldlines guarantee a full roster where round 3's lineage segments degenerated to 4 chains — plus the multi-seed certification of the pipeline over the disclosed field distribution at web-supplied rosters. Headline verbs of the form "the substrate reads out transverse geometry" are retired for this leg; the grade tag [DEMONSTRATED given Tier-A input] was carried correctly throughout (no false grading — the review's finding is wording, not substance).
- **MINOR-6 (G-3's status):** under Tier-A, lat ⊥ web *by construction* — the confound void is a wiring check (a placebo), not evidence: no channel exists by which the pipeline could read supply persistence. Its family-wise calibration (60 pair-tests at 2.5 sd) carries a ~48% per-run chance-void probability — conservative in direction, uncalibrated as a family; a max-statistic calibration is the named fix for the next leg. **The must-fail correlated-supply control — the amended gate's named void — belongs to the DERIVED-field leg** (ml3b's handoff), where a supply-χ channel actually exists; until that leg runs, **T6.4 stands at: L2 conjunct multi-seed-certified; the must-fail void OUTSTANDING** — the gate is not fully met on the paper trail.
- **MINOR-2 (transcription):** the LEDGER's shuffle-z range read "13.3–22.6"; the true range is **13.3–27.4** (the quoted top was the last printed row, not the max) — corrected at LEDGER #66.
