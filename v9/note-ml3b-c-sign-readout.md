...
## 5. Pre-receipt amendment (2026-07-06, committed BEFORE the receipt — the freeze discipline's amendment form)

**The anchor sector pair moves (0, 1) → (1, 2); (0, 1) demotes to a measured INFO row.** Foreseen while porting the machinery, before any step-(c) code existed: species A's original background Q = 0 is exactly parity-symmetric (free links), and for a free 2d overlap propagator S = A·1 + B_μγ_μ (the free overlap has no γ₅ component) the mixed bubble Π^{P,S} = −Tr[γ₅ S(u,v) S(v,u)] reduces to 2i·ε_μν B_μ B_ν = 0 **identically** — the ε-contraction of a symmetric product. Both A-side P-cells (E_PS, E_PP) are kinematically zero at Q = 0: Gc1 would refuse on a foreseeable algebraic technicality, not a physics verdict. Under the amendment both species sit on parity-breaking flux backgrounds (Q_A, Q_B) = (1, 2) (n_zero = 1, 2), where no such identity kills the cells; the (0, 1) row is printed as INFO — the measured kinematic parity-kill, the amendment's own receipt. Gc1/Gc2/Gc3 text is otherwise unchanged (anchor masses = the (1, 2) coupled solve; the free point = the (1, 2) decoupled masses). Tree-level structural note, also foreseen: the overall vertex factor g_x multiplies all four cells once, so the 4-cell parity is invariant under g_x → −g_x **except through mass back-reaction** — convention 3's print quantifies that residual channel.
