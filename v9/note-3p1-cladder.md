...
## Round-43 review corrections (appended) + the 43b receipt pin

**MINOR-1 (the F_iso reference card — pinned here, to be receipt-carried by 43b):** F_iso conflates in-plane anisotropy with cloud collapse. Review-derived references (verified numerically, pinned as 43b gates): exactly-planar in-plane-ISOTROPIC disk → F_iso ≈ 3.03 BY CONSTRUCTION (bottom directions starved by the missing dimension); eig-matched round-plane clouds: eig3/1 = 0.12 → ≈ 2.10, eig3/1 = 0.045 → ≈ 2.52; 3D-isotropic Gaussian → ≈ 1.07 (the M4 band [1.08, 1.13] is the honest round baseline). READING RULE: β ≥ 16 rows are collapse-dominated (F_iso ~ sqrt(eig1/eig3) scaling); their excess OVER the matched-eig round-plane reference (~50–150% here) is the residual in-plane anisotropy — the webs are collapsing AND remain non-round within the surviving plane. **MINOR-2:** the UNIVERSAL-vs-MIXED category boundary is threshold-sensitive at C = 4 equal-split (1.374 vs the 1.356 line — 0.018, inside seed noise); PARKING emptiness is robust (nearest point 10% above the candidate line). **MINOR-3:** "dimension climbs with C" carries [volume-proxy; out-of-family calibration at C ≥ 4; realizer certifies ≥ 3; witnesses not yet run]; C = 8 reads "≥ 5 (clamped)". **NIT-2:** the o9 < o4 anchor gap is paired-5/5 (sign test p ≈ 1/32, bands non-overlapping by 0.008) with a named mechanism ambiguity: CLT-Gaussianization vs 3-PCA PROJECTION rounding — 43b disambiguates. **NIT-1:** two-commit discipline resumes.

### Receipt 43b (`v9/code/dimwall_cladder_b.py`; pinned here, committed strictly before running)

- **Gr1 (the reference card):** five synthetic clouds through the F_iso pipeline verbatim (pinned seeds 20262500+, 5 each): 3D isotropic Gaussian; exactly-planar isotropic disk; round-plane + third-axis Gaussian at eig3/1 ∈ {0.12, 0.045}; a 3-simplex-interior cloud (the polyhedral reference). Gate: the measured card matches the review's derived values within seed bands (Gaussian ≈ 1.07 ± noise; disk ≈ 3.03; the two round-plane points bracketing 2.1/2.5; simplex above Gaussian and below disk-collapse readings printed).
- **Gr2 (the equal-split witness ladder):** S₄ search on C = 3 equal-split webs and S₅ on C = 4 (find_sk verbatim, tries = 20000, full-web relation, 5 seeds each; found ⇒ verify_sk); S₇ (C = 6) and S₉ (C = 8) as INFO at tries = 5000 (random search is underpowered at large k — disclosed; absence there is NOT evidence of absence). Gates: the S₄ and S₅ counts printed; witness-grade upgrade claimed only for what is found-and-verified.
- **Gr3 (projection-vs-CLT):** orthant-9's NATIVE 9-frame corner/face F alongside its 3-PCA F_iso (5 seeds, N = 1024): shadow-rounds-while-native-stays ⇒ the o9 < o4 gap is projection; both round ⇒ Gaussianization. INFO read, printed.
- Exit 1 only on Gr1 (a failed card = the instrument reference is wrong).
