...
From t23 §§3/5/6/8: (5) the ranking-measurability lemma (band counts are ranking-functionals — extends the race model to counts); (6) boundary-interval accounting (edge effects O(1/N)); (7) k-stratification (band uniformity); (8) the margins/variance instrument theorem (why the pooled instrument floors at ≈ 0.6 even for the box: the conditioned-margins variance deficit, closed-form scale E[α+β−αβ] ≈ 5/9 — the reference-floor discovery at its review-corrected attribution, t23 §9 C1; NOT a mean-model-bias effect); (9) the qualifying-class constant's lifetime-shape factor — **pending the t23 review's V3″ adjudication** (det/geom = 1.17 in-band vs the cv²-ordering in-pooled; the paper carries whichever resolution the review certifies).
