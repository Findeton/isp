# v12 — THE WELD: append-only ledger

House discipline in force (pins before receipts; exact arithmetic;
anchors exit-1-only; green-unreviewed not citable; hostile rounds
confer terminal; forward-only corrections; kill conditions are
engraved and fire without appeal).

## 2026-07-28 — FOUNDED: PAPER 0 + THE W1 PIN (v12 LEDGER #1)
User-authorized ("create v12, write a short paper 0 on this idea
then investigate W1") after the weld synthesis discussion.
**relativistic-isp-v12-paper0-the-weld.md** — the thesis
[PROGRAMME, not THEOREM]: T1 charts (every local description is
Barandes-classical), T2 the twist (the quantum content is a U(1)
cocycle on chart overlaps; interference = holonomy of description
loops; classical ⟺ trivial class), T3 records untwist (global
record = global section ⟺ trivial class — v11's arc as a
theorem-to-be, W3), T4 Born as forgetting the twist, T5 one world
fails by cohomology (BC1/BC2 as one class — a HYPOTHESIS, W4).
Evidence table over the committed corpus (v11 #14/#19/#21/#24, bc
#2-#4, D74's ⟨2,3⟩ ≅ ℤ², **U3's Hom(ℤ²,U(1)) = T² re-read as the
twist moduli**, v1's founding conjecture).  Antecedents engraved
(Fine; Abramsky-Brandenburger; gauge gluing) — v12 owns only the
weld: Barandes-as-charts, the generated T², the untwisting
mechanism, and the QUANTITATIVE claim (Tsirelson from U(1) cocycle
geometry).  Programme W1-W5 with kill conditions K1-K3 verbatim in
§4.  **note-w1-reconstruction-pin.md** — the executioner unit
frozen: the singlet from two classical charts + a U(1) cocycle and
nothing else; G1 the exact singlet instance (K1); G2 no-signaling
exit-1; G3 trivial cocycle = the classical polytope exactly (K3);
G4 THE CLASS MAXIMUM over U(1) instances (2√2 ⇒ the headline;
> 2√2 ⇒ K2 fires; < ⇒ W1-UNDER); G5 the group sweep {1}/ℤ₂/U(1);
G6 the T² correspondence sketch (reporting-only, feeds W2).  Lean:
the paper leans, THE PIN DOES NOT.  Context at founding: v11
halted at #24 (U1c delivered-uncommitted, unreviewed); BC halted
at #4 (BC2 repair killed mid-edit — bc2 receipt carries partial
...
## 2026-07-29 — PAPER 1 REPAIR VERIFIED — **PAPER 1 TERMINAL:
## PAPER1-PUBLISHABLE** (v12 LEDGER #33)
All 14 round fixes + the three adjudicated editorial decisions
applied; adjudicator verification: the three mechanical audits
ZERO (tokens — including the last "v12/"; rendering across 1,321
inline spans and 148 display delimiters; backslash-in-prose);
git shows the paper as the only modified file (the bundle
byte-untouched); spot checks — the V display in Definition 4.8,
the corrected φ-criterion sentence, Tsirelson 1987 J. Soviet
Math. 36 557–570 in [7], the repaired §3.5 table, the F6
first-pair scoping with "24 distinct values" — all present; the
abstract at half length; the repair's own bundle rerun 369/369
exit 0 (194.0s), and the renumbering validated mechanically (33
results consecutive per section; 95 references, 0 dangling; 17
reference entries, 17 cited, 0 orphans; 89 cross-reference sites
rewritten).  REPAIR DEVIATIONS ACCEPTED: (1) "24 distinct
values" is the paper's single unanchored printed number (the
bundle anchors the first pair only; the repair verified 24
independently in the bundle's own field and DISCLOSED it in the
appendix's Anchors subsection — the honest form); (2) the
γ-defining paragraph moved up together with the attribution
paragraph (definition-before-use); (3) [7] kept as one bracket
carrying both Tsirelson papers with roles named, avoiding a new
label syntax.  **PAPER 1 IS TERMINAL — PAPER1-PUBLISHABLE**: a
self-contained, externally-cited, mechanically-audited results
paper standing alone as a Barandes extension, with every number
regenerated by its exact bundle and its mathematics surviving a
full hostile round (no false theorem; 119 independent
