...
#156). THE OBJECT (the round-1 honest noun): the product-of-local-
conditionals WEIGHT SYSTEM on typed causal histories — the decision
half of a Harris factorization; NOT yet a measure (H2/H6),
...
def past_state(acts, pred, j):
    """The adjacency of the birth graph generated by event j's causal
    past — everything q needs (NO step counts are computed or used;
    round-1 m4(i) docstring fix), from past events ONLY."""
...
print("[d34a the Harris lemma — the product-of-local-conditionals WEIGHT SYSTEM]")
...
check("H2b THE RACE-vs-mu CONFRONTATION [exact, owned finding — round-1 "
      ...
      "uniform actor draw is NOT gauge, so the race's induced PATH LAW "
      "cannot "
      ...
      "an actual measure (d34b); the class-level pushforward tables are "
      "review-carried per the D33 precedent", ok2c,
...
check("H3 PERSISTENCE (the law-level claim is a coefficient audit: the "
      "1/4s are literals of q_local, size-free by inspection; ok3a is a "
      "single-instance probe — disclosed): the birth and interact masses "
      "are 1/4 and 1/4 at every local step of every record with a partner; "
      "a seeded race realization "
...
# THE COLLAR-LOCALITY LEMMA (round-1 delta, stated): the initiator's
# conditional depends only on ITS OWN adjacency as generated by its
# past-cone; events on another record's subtree change neither the
# initiator's collar nor its factors.
...
for h in HISTS:
    ...
    if any(op[1] in ('B', 'z8', 'z7')
           or (len(op) > 2 and op[2] in ('B', 'z8', 'z7'))
           for op in h):
