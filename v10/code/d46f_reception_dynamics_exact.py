...
check("RD4-f SELF-SCAN BY AST WALK (round-1 F-m4 / the campaign's "
      ...
      "name this source binds — SCOPE STATED EXACTLY (delta MA-2, the "
      "LOG #401 corpus obligation applied to this gate itself): a "
      "predicate that DOES reference a run-bound name while still "
      "being constant (e.g. check(len(STORE) >= 0)) PASSES this scan; "
      "that residual class is recorded for the authoring/pre-commit "
      "check. Two honest qualifications, so the "
