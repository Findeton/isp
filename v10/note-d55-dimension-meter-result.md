...
exit 0, outputs under `v10/data/`.  An independent Opus 5 hostile
round is required before anything here is citable.

## 1. A1 — the meter's reading

The generalized courier builder — **mechanical from `scd(m)`**, no
hand-tuning — first re-derived the m = 4 result as an anchor (22
actors, 44 events, all 16 subsets, shatter confirmed; same trace
family as D54's hand-built record), then answered the question:

> **A 42-actor, 84-event transport record, every event selected
> from the committed layer's own menu (10 accumulator chains, 26
> couriers), whose SKY-B sky realizes ALL 32 subsets of its 5
> pairwise-incomparable directions and returns a shattered 5-set —
> at depths 5, 6 and 7.**  Dilworth-consistent: per-initiator
...
- **A2a:** B₄ realized by 16 exact rational caps on S² (d47a's
  tetrahedron) — D54's record upgrades from "not an arc system" to
  **sphere-compatible**.
- **A2b [THEOREM, certificated twice]:** no five points shatter on
  S².  Exact affine dependences with non-empty sign splits on two
  rational configurations; the Radon argument closes it, and five
  points in R³ always carry a dependence.
- **A2c:** B₅ realized by 32 exact rational caps on **S³**
  (e₁..e₄ and (−½,−½,−½,−½)).

So the meter's scale is concrete: **the m = 5 record's sky fits S³
and provably not S² — beyond 3+1.**

## 3. Scope, held

Reading-relative to SKY-B; shattering depths enumerated (5, 6, 7).
ONE engineered record per m — **no genericity claim**; typicality
is not even posable at transport scope (D52).  "Fits S³" is
cap-realizability of the shadow family, nothing stronger.

## 4. What this changes `[MY READING]`

The geometry line is now complete in one direction: the framework's
admissible records can carry skies of arbitrary-looking sphere rank
(m = 4 → S², m = 5 → S³; the builder pattern visibly generalizes,
...
1. The general-m claim (shatter-m for all m by this builder) —
   visibly patterned, not gated; a receipt at m = 6 (20 chains,
   57 couriers, ~66 actors) would strengthen it, or a proof that
   the builder is admissible for every m would settle it.
2. Minimality at every m (the gate's C(k,⌊k/2⌋) vs the builder's
   spend).
3. The convergence question itself, pending B1.
