...
## 5. Round-1 outcomes (fronts pinned above; 2026-07-12; LEDGER #130)

(F1) **CLOSED — the verdicts are measure-independent:** every ρ_η is ½I ⊕ (αI + βX) on span{|000⟩, |111⟩}; the whole family commutes (shared eigenbasis (|000⟩ ± |111⟩)/√2, eigenvalues (1±η)/2), and all five closures map it to commuting families, so every monotone distinguishability measure reduces to the classical one on eigenvalue vectors — the R2 verdicts hold for all of them. (F2) **CLOSED by correction:** Molnár/Kadison assume surjectivity, which the into-bigger-space closures violate; the governing theorem is **Busch 1999** (trace-norm-isometric completely positive TP maps = mixtures of isometries with mutually orthogonal ranges — Thm 2; the positive-only Thm 1 adds antiunitary branches, invisible on the real family), and D27's thermal-receiver exhibit U(ρ⊗σ)U† — distance-preserving, pure→mixed, exactly Busch form — refutes this note's original "= exactly the isometric dilations" while leaving every exclusion intact (supersession at LEDGER #130; paper 19 §3 carries the corrected characterization). (F3) **NO false exclusion:** the condition is closure-scoped at every statement site; the thermal receiver — the paradigm Q-class member — is admitted (D27 B1). (F4) **Scoped:** the receipt computes a TWO-way split (value preserved, content killed); the occurrence leg is definitional, carried by the record register. Review notes applied: the value pair alone separates none of the five closures — the η-pairs carry all discrimination; §2's "verifies the block structure" means "verifies the trace-condition certificate", and that certificate is now PROVEN sound (review theorem: tr D³ = 0 ∧ (tr D²)² = 2 tr D⁴ ⇒ nonzero spectrum exactly {x, −x}; no false certificate exists); C_half's contraction is (1−λ)D at rate λ — the receipt's D/2 is the λ = ½ representative.
