...
(F1) Is `Delta^3 = c·Delta` + rationality of sqrt(c) a complete certificate for the family's differences (any case silently uncertified?). (F2) The orthogonal-ranges check: is range-orthogonality verified in the right inner product (it is the plain Euclidean one on real vectors here — confirm adequacy). (F3) Does the "flag receives nothing" gate need the joint (flag, receiver) marginal rather than the flag weights alone?
