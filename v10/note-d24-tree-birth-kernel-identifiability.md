...
## 4. Round-1 outcomes (fronts pinned above; two internal streams + one external, 2026-07-12; LEDGER #130)

(F1) **ANSWERED and upgraded:** G3 now gates the isometry-grade ledger — the preparation overlap ⟨ψ_a|ψ_b⟩ exactly preserved at every birth (the D25 distinguishability form; mere state-inequality was the injectivity form D25 proved too weak). (F2) **CLOSED — the general lemma is TRUE and proven** (review): any two causal linear extensions are connected by transpositions of incomparable edge pairs; an incomparable pair shares no register or only the control; both kinds commute (shared-control edges are block-diagonal in the control's Z basis with disjoint targets). Receipt upgraded to exhaustive: every labeled rooted tree on 4 registers, all 36 causal extensions, one law each; the review's clean-room run extends to 141 trees / 612 extensions on 4–5 registers, zero mismatches. (F3) **HONEST and now sharper:** the g = 1 boundary genuinely breaks the zero-pattern criterion (review: a sibling reads as an ancestor) — the interior-coupling scope is load-bearing; recovery is now CLICK-SIDE ONLY in the receipt (the round-1 truth-side-circularity in the parent rule fixed; the rule itself is provable — ancestor sets along a root path are nested — and review-verified on 864 recoveries through n = 5 including adversarial equal couplings). (F4) **The delta, named:** the isometric growth semantics (G1/G3), the gauge theorem (G2), the reduction gate (G4) — and, per the external review's register-ontology question, G4 IS the growth-versus-activation equivalence: appending a fresh |0⟩ factor and activating a pre-supplied frozen mode are click-equivalent (paper 19 §2). Wording fixes applied: G5 uses one fixed interior assignment (B: 9/25, C: 16/25, D: 576/625), not a sweep (the review's sweep — all 27 assignments at n = 4 plus equal couplings — found zero collisions, so the stronger reading is also true); "byte equality" means exact Fraction-dict equality.
