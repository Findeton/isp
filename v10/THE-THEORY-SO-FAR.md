...
> as of **LEDGER #480** (v10/LOG.md) / **#130** (v8/LEDGER.md).
...
11. Both lines therefore arrive at the same wall: *does anything prefer
    ...
    **named missing map**, so until it closes the grammar's geometry and
    the laboratory's clicks are two ledgers — though two segments of that
    ...
    **not** descend to a measure on records, by exactly one repairable
    ...
    coherent, and where superposition enters is now the programme's
...
- **A2. The click law** — the six kinds of thing that can happen; what
  ...
  programme's other stream — the identified law; and the two measured
...
**What that leaves, and it is the programme's sharpest open question.**
Superposition must enter somewhere else.  There are three named places
and no evidence between them: at **transport scope** — where deliveries
...
cheapest to attack and nobody has attacked it.
...
> missing map — of which §§A2.7–A2.8 measure two segments, from the
...
checked was non-empty.  The five most recent retirements above are the
...
receipt.*  The three most recent cases also show what that obligation
buys when it is met: one refutation of a width sentence arrived with a
**better record** attached; the next killed a *negative* — a limit the
unit had announced — and left the strongest geometry statement the
programme has; and the last inverted **both** of a unit's headline
sentences and returned, in their place, the sharpest fact the programme
owns about its own quantum layer *and* the sharpest open question it now
has.  **Being wrong in the pessimistic direction is a failure mode too,
and this list now contains both kinds.**

**And the last one adds a third kind, worth naming separately: a
convention wearing the clothes of a result.**  Both withdrawn sentences
were true *of a choice the unit had made about how to write its own
equations* and false of the thing being studied.  The check that catches
that failure is not another control; it is going back to the source the
demand was quoted from and writing the demand as it is actually stated.
...
stopped being only a name.  Two of its segments are now measurements
(§§A2.7–A2.8): the generated law's normalized weights do not descend to a
probability of a record, by one repairable defect, while the settled
completion's do; and the quantum layer such a law could carry has been
counted rather than speculated about.  The second measurement is the more
consequential and the less comfortable — what it found is that
consistency with the records is far too weak to organize interference,
and that the first demand asking the interference to come from the law
itself removes all of it.  So the map's functional stretch is **narrower
and harder** than it was: narrower because it has been measured, harder
because the measurement's answer is a negative at the only scope where
the corpus can compute.
...
1. **Where does superposition enter?**  At the one scope where it can be
   ...
   *one* of the three ways of writing the record condition, and its
   uniqueness was not established — so the cheapest next move in the whole
   programme is to run the same demand against the reading that is
   actually stated, and then against a second demand.  Beyond that the
   named homes are the delivery scope and a different joint of the map
   altogether.

2. **A bounded description of the theory at delivery scope.**  The one
   remaining gap in the wall (chapter A9), and what unblocks the
   convergence question — and, since the question above points at that
   same scope, now the second thing blocked on it.  Exact per-option
   descriptions are impossible for
...
3. **Which sky reading is physically privileged?**  All three are live,
   ...
   it.
4. **What is left of the width road.**  The object three separate
...
5. **Is there a record-level demand that forces the shape?**  The two
   ...
   (§A2.7).
6. **A machine-checked version of the delivery-free settlement.**  Both
...
7. **General rungs of the ladder.**  The builder visibly generalizes
   past five; it has not been run, and unrun is unclaimed.
8. **Minimality.**  Six actors are necessary for shatter-four and twenty
   were spent; ten are necessary for shatter-five and forty-two were
   spent.  Both gaps are architectural and both are decidable.
9. **The residual pricing defects.**  The off-ladder configuration; the
...
10. **The reading-relativity questions.**  Which sky definition is
...
- **Hostile rounds, and now independent-model rounds** — a reviewer with
  ...
  its own code.  Ten such rounds have run; eight found a blocker; **all
  eight blockers were in interpretation, not arithmetic**, and the five
  most recent were the same mistake five times — a headline built on
  numbers that were all correct, refuted by a computation, or a
  construction, the unit had not thought of.  The last three came
  ...
  had announced and left the stronger claim in its place; and one that
  inverted **both** of a unit's headline sentences and returned the
  programme's first hard fact about its own quantum layer, together with
  the question that now ranks first.
...
families — delivery, conflict, crossed conflict — answering no.  (2) **The ambiguous pairs** — 27 of 172 at
...
- **Almost every headline correction hit an interpretation sentence, not a
  computation.**  Ten independent-model rounds have run (D54, D55, D61,
  D62, D63, D64, D65, D66, D67, D68); eight returned a BLOCKER and
  **every one of the eight was in the interpretation layer, none in the
  arithmetic**.  In D54
  ...
  **identical and forced**.  D64, D65, D66, D67 and D68 are the same
  failure five times: the referee reproduced *every* number and then ran —
  ...
  then **one more round of it** (§B8.9), and the parent paper's own
  decoherence condition plus the dynamical demand the unit's own residue
  had named and declined (§B2.11) — and each refuted the headline built on
  the numbers it had just confirmed.  **A
  verified computation is not a verified claim**, and the corpus's
  standing lesson from these five is to ask what computation would
  *distinguish* the headline from its trivial alternative, before writing
  the headline.  The last three also show the upside of that discipline,
  and each is sharper than the last: D66's refutation arrived with a
  **better object** attached; D67's killed a **negative** the unit had
  ...
  programme has; and D68's inverted **both** headline sentences of a unit
  and returned the programme's first hard fact about its own quantum layer
  together with its sharpest open question.  **A unit can be wrong
  pessimistically, and the round that catches it returns more than it
  removes.**
- **D68 also names a distinct failure mode: a CONVENTION wearing the
...
**The map between the lines — two measured segments, both from the
generated side, neither transferring.**  §B2.10: the normalized menu
...
truncation-bound; **DC3(2) is narrowed, not discharged.**
...
2. **Residue 2, REOPENED: which sky reading is physically privileged?**
...
5. **The missing map's functional segment — MEASURED at closed scope, and
...
corrections only, never silently edited.**  Entries #404–#480 cover
everything in chapters B5, B7, B8, B9, the settlement line of B6 and the
descent and functional-slot segments of B2.
...
- **Hostile rounds, and now independent-model rounds** — a reviewer with
  ...
  own code.  Ten have run; eight found a BLOCKER; **all eight BLOCKERs
  were in interpretation, none in arithmetic** (§B10.17).
- **Ask what would distinguish the headline from its trivial
  alternative** — the lesson D64, D65, D66, D67 and D68 taught five times
  in two weeks.  All five units measured correctly, reported correctly,
  and read
  ...
  condition written as the parent paper states it).  The obligation is now
  on the pin: *name the
  computation that would make the interesting reading false, and run it in
  the same receipt.*  The last three add the corollary that meeting it can
  **pay**: the construction that refuted D66's headline became the
  ...
  statement; and the round that refuted both of D68's spent the ten lines
  that unit's own residue had named and declined, returning the
  programme's first hard fact about its quantum layer.  **The obligation
  covers negatives too — a limit announced is a claim, and it is cheapest
  to test by extending your own table.**
...
is what makes the emptiness legible**: every substrate whose transition
maps have been measured — delivery crystals, conflict crystals, and the
crossed-conflict double grid built precisely because a conflict event
lacks the symmetry that trivializes a delivery — has a class that is a
**coboundary** (§B8.8, §B8.9), so there is no non-trivial transformation
behaviour to certify.  The doctrine names the deliverable; the deliverable
does not exist yet, and the search for it has now failed on the substrate
that had the best structural reason to supply it.
...
the route that looked most likely — and the quantum layer's own
elimination (§A2.8) has just added a third thing waiting on that same
scope.
...
closes, none of this reaches a laboratory even in principle.  Two of its
segments are no longer names but measurements (§§A2.7–A2.8), and the
...
mechanism lives there.  The measure defect points there.  And now
coherence points there too, by elimination.  Three unrelated threads, one
address — which is the most useful thing the destination has learned in
this stretch, and also the reason the scope that has no probabilistic
description is now blocking three things instead of two.
...
be both state-generated and coherent.  Four of those six are negatives or
carry one.  All are results, all are terminal, and each changed the
...
successor, and now by turning the programme's widest gap from an
emptiness into a measured negative with an address.

**Re-ranked against the destination** (superseding §A11.4 / §B11.4's
ordering, not their content):

1. **THE MISSING MAP'S FUNCTIONAL LEVEL — THE DEMAND'S UNIQUENESS, AND
   THE SAME SLOT AT TRANSPORT SCOPE** (§B2.9, §B2.10, §B2.11) — the
   corpus's **deepest** named front and still its **widest**, and the
   only item on this list whose next two moves are *named computations*
   rather than a method problem.  It ranks first because the construction
   that used to outrank it is **finished** (item 4), because it is the one
   gap that makes two ledgers out of one programme, and because the
...
2. **TRIPLE-GRID — the one named construction candidate left, and it is a
   candidate, not a result** `[NOT BUILT]`.  Every crossed-conflict record
   in the corpus crosses **two** conflict axes (§B8.9).  Nothing in the
   grammar stops a third, and three questions that are currently answered
   only at two axes would separate at three: whether the **dimension
   profile** past depth 2 is a function of the number of axes rather than
   of `k` alone (the `k²` ceiling is a depth-2 statement, and the
   depth-`d` bound `Bl^d` is not known to be tight); whether a third axis
   produces **loop structure the gluing can see**, which is the only
   untried structural reason left to expect a non-coboundary class after
   three substrate families said no (item 5); and whether crossing
   **selects a `k`** — every ceiling so far is realized at whatever `k`
   the builder picked, so nothing in the corpus explains why a record
   would have one dispute size rather than another.  It ranks here
   because it is cheap, because it is the only place a *construction*
   could still change a negative into a positive, and because two of its
   three questions are the two the destination most needs answered.
   **No number is claimed for it; it has not been built.**
...
4. **WHAT IS LEFT OF THE WIDTH ROAD — two proofs of reach, not a
   ...
   *Settled:* conflict **tiles** — 21 three-proposer and 11 four-proposer
   ...
   the gluing motivation is spent (item 5 below).  *Unbridged:* the
   ...
   **height + 1** — which a levelling pass supplies without any crossing
   ...
   to make space.**
5. **THE GAUGE QUESTION, and its honest state.**  *Can any record at all
   ...
   **Three substrate families** — delivery circuits (§B8.8), pair-conflict
   ...
   and exactly **one** untried site is named anywhere: a **third**
   concurrent conflict axis (item 2), on the thin structural hope that
   crossing three ways gives the nerve loops that crossing two ways does
   not.  That is a hope, not a lead, and if it also returns a coboundary
   the gauge road has no candidate site left at all.
6. **A workable probabilistic description at transport scope**, unchanged
   ...
   untested.
7. **The two empty bridges (§C5, §C6.11).**  Arrow 4 —
   ...
   *declares* them not to be the click law.  **Two unbridged bodies of
   work, one theory.**
8. **A Lean-grade mechanization of the delivery-free settlement.**  The
...
measure defect's own successor question is a transport-scope one; and
the functional slot, having been measured at closed scope and found to
admit no state-generated coherence, hands superposition to transport
scope by elimination.  Four fronts, one place — and that place is
precisely the one where the corpus provably has no exact description
(§B9.1) and where its best remaining candidates are untested
aggregations.  **That coincidence, not any single residue, is the
programme's real bottleneck.**

Against it, the ledger of moves is short and honest.  There is **one**
concrete construction move outstanding — *find a coarser aggregation that
closes at transport scope* — and **one** cheap named computation, the
uniqueness of the dynamical demand (item 1a), which is the only place a
receipt could change the quantum verdict without new machinery.  There is
**one** candidate construction that has never been tried and could still
turn a negative positive, the third conflict axis (item 2).  The rest are
...
proof-shaped residues (general `k`; a tiling above `k = 4`).  And two
fronts that had a named next move no longer have one: after three
substrate families, five conventions and three routes the search for a
**non-coboundary gluing** has no candidate site left except the untried
third axis, and after the ceiling was taken at four consecutive sizes
...
> across three substrate families at five conventions, and exactly one
> untried site is named, a **third** concurrent dispute axis — and two
...
> map**, both of whose measured segments are negatives with addresses: the
...
> measure does; and the quantum layer such a law could carry, counted
...
> line still has no *derived* functional level at all.  Where superposition
> does enter — a different fair demand, a different joint, or **transport
> scope**, the same address the dimension mechanism and the measure defect
> already point at — is now the programme's sharpest open question.
...
| **the missing map** | paper 29's name for what stands between the two streams: *"the action line and the generated-law line now meet at one missing map"*.  The corpus **supplies rather than derives** the identified law's boundary state, measure, record instrument and **generated record grammar**, so that law is **not yet proved record-closed**.  Partially bridged by the conditional descent theorem; the corpus's deepest named front.  **Two segments are measured**, both from the generated side only: the descent defect (§B2.10) and the **functional slot** (§B2.11).  The second **narrows and does not discharge** the widest stretch — there are still no *derived* class operators and no *derived* Gram functional, so paper 29's decoherence hypothesis still holds *for the empty reason*, but the space of candidates is now exactly counted and carries one hard negative | B2.9, B2.10, B2.11, D4 |
...
10. **No new computation was performed for this document.**  Every number
    ...
    **D63** (§B8.7, the wide crystal and the branching bound) likewise,
    ...
    had named and declined, which **inverted its verdict**.  All eight
    round 1 terminal.
...
15. **Five things this document leaves open, and a reader should not
