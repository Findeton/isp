...
This note extends the private finite-alphabet cut-capacity candidate to
...
Thus the hard ensemble is fixed by the experiment family before any positive
cut model is inspected. The quantifier order is
...
Radhakrishnan, Lemma V.3. For a one-way private-coin protocol with finite
message \(Q\), input law \(\pi\), average error \(e\), and any \(\Delta>0\),
their result at one round gives

$$
D^\pi_{e+\Delta}(f)
\le
\frac{2I(X,Y:Q)+O(1)}{\Delta}.
$$
...
Let \(\Lambda\) be a standard-Borel cut carrier. An \(S\)-indexed bundle
\(\Lambda_s\) is allowed only when its total space is standard Borel and its
projection to \(S\) is measurable, so that \((S,\Lambda)\), all kernels below,
and conditional mutual information are well defined. A continuous positive cut
model supplies:
...
Because every response kernel is jointly measurable in \((s,\lambda)\), for
every pair \((s,\lambda)\) define its complete binary response vector

$$
r(s,\lambda)
=
\bigl(
\xi_{y,s}(1\mid\lambda)
...
3. She samples \(\Lambda\sim\mu_{x,S}\).
...
Equivalently, the induced protocol has a fixed positive slack
...
Then, for every admissible \(n\) with \(\alpha n\in\mathbb N\), every
standard-Borel positive cut model in Section 4 obeys, under the natural hard
distribution \(\pi^{\rm nat}_{n,\alpha}\),
...
\(I(X,Y:Q_\eta)=I(X:Q_\eta)\). Apply Harsha--Jain--McAllester--
Radhakrishnan Lemma V.3 with any fixed
\(0<\Delta<\delta_*-e\). It gives

$$
D^{\pi^{\rm nat}_{n,\alpha}}_{e+\Delta}(f_{n,\alpha})
\le
\frac{2I(X:Q_\eta)+O(1)}{\Delta}.
$$
...
Rearranging, and using that \(\Delta\) is fixed independently of \(n\), yields
...
Suppose the full preparation \(x\) is injected into one real coordinate

$$
\lambda=f(x).
$$

If this coordinate is sufficient for every later matching response, its mutual
information under the hard ensemble is charged. An injective exact encoding
typically carries all of \(H(X)\), and in any case the theorem forces at least
...
The private model-neutral contract for testing that obligation is now recorded
in `/private/tmp/v17_uniform_indivisible_law_classification_gate.md`. Its root
...
| fixed-slack quantifier | candidate pass | \(\delta_*-(\delta+\varepsilon_{\rm fac}+\eta)=\gamma>0\) is fixed independently of \(n\), so message compression preserves the asymptotic constant |
...
- D. Grier, H. Pashayan, and L. Schaeffer, “Sample-optimal classical shadows
  for pure states,” *Quantum* **8**, 1373 (2024), especially Sections 4.2.1--
  4.2.2, <https://doi.org/10.22331/q-2024-06-17-1373>.
- D. Gavinsky, J. Kempe, I. Kerenidis, R. Raz, and R. de Wolf,
  ...
  applications to cryptography,” <https://arxiv.org/abs/quant-ph/0611209>.
