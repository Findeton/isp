# ISP v17 — phase-complete composition investigation

## From indivisible endpoint laws to one uniform quantum nomology

**Status:** PRIVATE / RESULT-NEUTRAL INVESTIGATION / NOT A PIN  
**Date:** 2026-08-23  
...
Therefore the passage from phase-complete processes to positive endpoint
matrices is not compositional. The positive quotient forgets precisely the
relative information later revealed by coherent continuation.
...
$$
\begin{array}{ccc}
\text{phase-complete processes}&\xrightarrow{\ \circ\ }&
\text{phase-complete processes}\\
...
## 3. Composition-connection formulation

The fiber $q^{-1}(G)$ contains many phase lifts. A pointwise lift is
insufficient because independently choosing representatives need not respect
wiring. The candidate mathematical object is a gauge-covariant composition
connection over the typed experiment category.
...
operational equivalences and ontology costs rather than select one by taste.
...
Prove and review the E-Comp theorem, including the coherent $U_\theta$ fiber,
carrier-relative rank obstruction, actual measurement control, and gauge wall.

### PC2 — lift classification on a finite separating grammar

For a frozen finite gate/instrument grammar, classify all uniform
phase-complete lifts or direct whole-law extensions of the positive primitive
...
Any future composition-connection gate must include:
...
The composition connection, if it exists, would answer only how a quantum law
maintains phase-complete predictive structure under composition. It would not
yet answer:
