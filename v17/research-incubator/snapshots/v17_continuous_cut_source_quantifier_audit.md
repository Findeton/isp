...
**Status:** PRIVATE / NONBINDING / AUTHOR-SIDE AUDIT / NOT INDEPENDENT REVIEW
...
This note reconstructs the external theorem chain and attacks the reductions in
`/private/tmp/v17_continuous_cut_information_theorem_candidate.md`. It does not
...
After these repairs, no counterexample was found to the following restricted
claim:
...
HJMR Lemma V.3 states, for any possibly correlated distribution \(\pi\) on
...
At one round, for a particular finite message \(Q\), this gives
...
Grier, Pashayan, and Schaeffer restate this one-way consequence in their
Theorem 23 and use it to obtain an information lower bound for the same hidden
matching family. Their reference [25] is HJMR. The previous candidate's direct
attribution to Jain--Radhakrishnan--Sen was therefore inaccurate.

Secondary application source:
<https://doi.org/10.22331/q-2024-06-17-1373>.
...
The constants \(\alpha,\delta,\varepsilon_{\rm fac},\eta,\delta_*\), and a
...
Let the total carrier of \((S,\Lambda)\) be standard Borel and require every
binary response probability

$$
(s,\lambda)\longmapsto\xi_{y,s}(1\mid\lambda)
$$
...
1. the constant-error extension of the natural-distribution lower bound for
   every printed \(\delta_*<1/2\);
2. the exact one-round specialization of HJMR Lemma V.3, including the
   additive \(O(1)\) and truncation convention;
...
```text
SOURCE CHAIN:                         REPAIRED / PROVISIONALLY PASS
...
OFFICIAL PIN READINESS:                PENDING INDEPENDENT-REVIEW CONTRACT
```

The appropriate next private artifact is a result-neutral candidate pin that
freezes one representative constant, preferably \(\alpha=1/4\), the natural
hard ensemble, the exact positive-cut class, fixed error slack, and all
premise-escape branches. It must disclose that an author-side proof candidate
already exists; later review can be independent verification but not blind
discovery.
