...
**The dimensionless `10^{−120}` is the *only* place `l_step` (= `G`) enters — and there it is a tautology.** Expressing Λ in Planck units forms the ratio `Λ·l_step² = (l_step/R_H)²`; with `V = R_H^4` this is `δΛ = 1/√V = R_H^{−2} ≡ 1/R_H² = Λ_obs` *identically* (`log₁₀(predicted/observed) = 0.0`, `|·| < 10^{−90}`; the match algebraically **forces `d = 4`**). The famous `10^{−120}` is then just the one measured ratio `R_H/l_step ∼ 10^{60.93}`, squared — the universe's size in Planck lengths, restated, not a derived number (`N = (R_H/l_step)^4 ∼ 10^{243.7}`, `δΛ ∼ 10^{−121.86}`).
