...
2. **The swerve's existence is inherited, not invented.** The Lorentz-invariant momentum diffusion is the causal-set "swerves" of Dowker–Henson–Sorkin (2004) and Dowker–Philpott–Sorkin (2009). What is **SHARD-specific** is the *tie of the diffusion constant to the seal rate* `σ` (the structural relation `κ·l_step³/σ = 1/6`) — and the tie is a **posited identification, not a derivation**: its two inputs (per-seal kick variance `l_step⁻²` with unit coefficient; crossing rate `ν = σ/l_step`) are ansätze, so the owned content is the proportionality `κ ∝ σ·l_step⁻³` with a model-dependent `O(1)` coefficient (receipt `p19c_swerve_fdr.py`, 19/19: the de Bruijn identity `dS/dτ = κ·I_F` grounds the proportionality thermodynamically yet confirms the bridge is posited; its relativistic-Einstein completion would make `κ` mass-dependent — a flagged tension). The records carry the swerve because the records *are* the causal set (Paper 1); the seal-rate tie is the new content.
...
So the *shape* of the prediction is record-native — the swerve constant proportional to the seal rate in cell units — while the identification itself is **posited, not derived**: the per-seal kick variance `q² = l_step⁻²` (unit coefficient) and the crossing rate `ν = σ/l_step` are ansätze, so the `1/6` is kinematic (`1/(2(d−1))`) and the coefficient model-dependent `O(1)`. A thermodynamic grounding exists — the swerve is a heat flow, so `dS/dτ = κ·I_F[f]` (de Bruijn) makes `κ` the substrate's entropy-production-per-Fisher-information coefficient, and identifying that entropy production with the seal current recovers the proportionality (receipt `p19c_swerve_fdr.py`, 19/19, `dps = 100`) — but the bridge remains one posited identification, and its relativistic-Einstein completion (`κ = νMT_sub`) would make `κ` mass-*dependent*, in tension with the mass-independent tie; flagged, not resolved.
...
(receipt `p19a` check 5, arithmetic at `dps = 140`, flagged; with `l_step = l_Planck` this reads `σ < ~3×10⁻¹¹⁸`, conditional on the import, comfortably satisfied). We **constrain** `σ·l_step⁻³`; we do not predict `κ` or `σ` — exactly the move *Testing ER=EPR with Hydrogen* makes for the ER=EPR amplitude. One consistency question is left open rather than presumed: whether the corpus's own readings of `σ` along a massive worldline (the dense click-law regime; the seal-decoherence rate law) *satisfy* this bound at `l_step = l_Planck` is unexamined — the constraint either bites the substrate's parameter space or is vacuous, and settling which requires the (open) supply law for `dχ` (Paper 1 §5.1).
...
hence **CP-divisible**, **monotone non-increasing**, with **no revivals**, and the Gaussian onset `λ ~ t ⇒ |ρ_{01}| ~ exp(−κ_G t²/2)` near `t = 0` (Paper X; `κ_G` the onset curvature, not Channel A's swerve constant). CP-divisibility is incompatible with Barandes-indivisibility — a genuine structural fingerprint of the seal.
...
| The seal-rate tie `κ = (1/6)σ·l_step⁻³`, relation weight-0 | `SHARD-SPECIFIC / POSITED` (identification, not derivation; grounded thermodynamically in `p19c`) |
...
- S. Donadi et al., *Underground test of gravity-related wave function collapse*, Nat. Phys. **17**, 74 (2021) — the Gran Sasso X-ray bounds ruling out the parameter-free Diósi–Penrose collapse rate, constraining the decoherence channel.
