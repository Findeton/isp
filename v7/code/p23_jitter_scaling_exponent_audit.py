#!/usr/bin/env python3
"""
Collapsed Paper 23 receipt: jitter scaling exponent audit.

The previous phase diagram fixed hidden width 4 and varied the effective jitter
scale.  This receipt asks whether the dangerous boundary is better described by
a fixed jitter radius, a sqrt(N) jitter radius, or a weakly linear jitter radius,
and whether that answer changes with hidden width.
...
rooted near-neighborhood features.  It is a finite exponent audit, not an
asymptotic theorem.
...
print("=" * 80)
print("Collapsed P23 jitter scaling exponent audit")
print("=" * 80)
...
def schedule_effective(label, N):
    ...
    if label == "fixed_16":
        return 16
    if label == "sqrt_half":
        return max(1, int(mp.nint(mp.sqrt(N) / 2)))
    if label == "sqrt_one":
        return max(1, int(mp.nint(mp.sqrt(N))))
    if label == "sqrt_two":
        return max(1, int(mp.nint(2 * mp.sqrt(N))))
    if label == "linear_24":
        return max(1, int(mp.nint(mp.mpf(N) / 24)))
    raise ValueError(label)


schedule_labels = ["fixed_4", "fixed_16", "sqrt_half", "sqrt_one", "sqrt_two", "linear_24"]
...
for width in widths:
    print(f"\n(2) Testing clustered coordinate laws at hidden width w={width}")
    for N in Ns:
        scale_result = {
            "width": width,
            "N": N,
        ...
        for label in schedule_labels:
            ...
            print(
                ...
                f"same_near={fmt(observable['same_near'], 18)} phase={phase} top=[{top}]"
            )
...
phase_changes_by_width_label = []
for width in widths:
    for label in schedule_labels:
        phases = [candidate["phase"] for _, candidate in rows_for(width=width, label=label)]
        if len(set(phases)) > 1:
            phase_changes_by_width_label.append((width, label, phases))

width_sensitivity = []
for N in Ns:
    for label in schedule_labels:
        ratios = [candidate["score_ratio"] for _, candidate in rows_for(N=N, label=label)]
        if min(ratios) > 0 and max(ratios) / min(ratios) > 2:
            width_sensitivity.append((N, label, min(ratios), max(ratios)))

largest_N_camouflage = [
    (row["width"], candidate["label"], candidate["score_ratio"])
    for row in rows_for(N=max(Ns))
    for candidate in row["candidates"]
    if candidate["score_ratio"] <= 1
]
largest_N_visible = [
    (row["width"], candidate["label"], candidate["score_ratio"])
    for row in rows_for(N=max(Ns))
...
fixed4_largest = [candidate["score_ratio"] for _, candidate in rows_for(N=max(Ns), label="fixed_4")]
sqrt_two_largest = [candidate["score_ratio"] for _, candidate in rows_for(N=max(Ns), label="sqrt_two")]
...
check(
    "At least one exponent schedule changes phase across tested N at fixed width",
    len(phase_changes_by_width_label) >= 1,
    "; ".join(f"w={w} {label}:{'/'.join(phases)}" for w, label, phases in phase_changes_by_width_label[:6]),
)
check(
    "Width is an independent control parameter at fixed N and exponent schedule",
    len(width_sensitivity) >= 1,
    "; ".join(
        f"N={N} {label} min={fmt(lo, 8)} max={fmt(hi, 8)}"
        for N, label, lo, hi in width_sensitivity[:6]
    ),
)
check(
    "The largest tested scale still contains both visible and camouflage regimes",
    bool(largest_N_camouflage) and bool(largest_N_visible),
    f"camouflage={[(w, label, fmt(r, 8)) for w, label, r in largest_N_camouflage[:4]]}; "
    f"visible={[(w, label, fmt(r, 8)) for w, label, r in largest_N_visible[:4]]}",
)
check(
    "Fixed small jitter is not uniformly washed out at the largest tested scale",
    any(ratio > 1 for ratio in fixed4_largest),
    ", ".join(f"w={widths[i]} fixed_4={fmt(ratio, 10)}" for i, ratio in enumerate(fixed4_largest)),
)
check(
    "Large sqrt-scale jitter is not uniformly separated at the largest tested scale",
    any(ratio <= 1 for ratio in sqrt_two_largest),
    ", ".join(f"w={widths[i]} sqrt_two={fmt(ratio, 10)}" for i, ratio in enumerate(sqrt_two_largest)),
)

print("\n(4) Consequence")
print("The exponent audit keeps the phase-boundary problem alive but makes it")
print("sharper.  The dangerous parameter is not just raw jitter.  Hidden width,")
print("effective rank-mixing radius, and N all matter.  Fixed small jitter can")
print("remain order-visible at the largest tested scale, while sufficiently large")
print("sqrt-scale jitter can still sit inside the finite null for some widths.")
print("The theorem target should therefore classify Q_N(w_N, j_N), not only")
print("fixed-width or fixed-jitter families.")
