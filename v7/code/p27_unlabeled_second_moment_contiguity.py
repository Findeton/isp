#!/usr/bin/env python3
"""
Paper 27 receipt: unlabeled second-moment/contiguity campaign.

The frozen adversary is Q_N(2,c): replace each base 1+1 sprinkling point by
two records, then jitter the two records through a coordinate window cN before
forgetting the hidden pair labels.  The target object is the unlabeled
second moment

    S_N = E_{P_N}[(dQ_N/dP_N)^2] = sum_o q_N(o)^2 / p_N(o).

Exact P_N is only feasible here at N=8, so the receipt does three honest
things:

  - computes exact unlabeled P_8 by enumerating all 8! coordinate permutations;
  - estimates S_8 with two independent Q samples, sum q_A(o) q_B(o) / p(o);
  - follows the rare-class opening by accumulating the largest positive
    order-class contributions across repeated split samples.

This is finite evidence, not an asymptotic proof.  The hostile pass condition
is deliberately narrow: sharp and sublinear/quarter clustering must be visible,
while linear half/one/two schedules must remain inside the exact-P calibrated
split-null guard if the washout interpretation is the honest one.
...
def sample_clustered_counts(n, sample_count, seed_base, jitter_num, jitter_den):
    rng = random.Random(seed_base)
...
def exact_zero_jitter_counts(n):
    blocks = [tuple(sorted(edge)) for edge in fixed_matching(n)]
    width = 2
    scale = 10000
    counts = Counter()
...
def accumulated_top(rows):
    ...
    out = []
    for code, contribution in ordered[:6]:
...
print("=" * 80)
print("Paper 27 unlabeled second-moment/contiguity receipt")
print("=" * 80)
...
relabel_ok = canonical_unlabeled_code(spot) == canonical_unlabeled_code(relabel_poset(spot, 27000001))
print(f"permutations={mp.factorial(n)} support={len(p_law)} relabel_spot={relabel_ok}")

print("\n(2) Exact zero-jitter baseline")
sharp_counts = exact_zero_jitter_counts(n)
...
for rep in range(null_reps):
    counts_a = sprinkled_counts(n, sample_count, 27100000 + rep * 20000)
    counts_b = sprinkled_counts(n, sample_count, 27200000 + rep * 20000)
...
for schedule_index, (name, jitter_num, jitter_den) in enumerate(schedules):
    ...
    for rep in range(rep_count):
        counts_a = sample_clustered_counts(
            n,
            sample_count,
            27300000 + schedule_index * 200000 + rep * 20000,
            jitter_num,
            jitter_den,
        )
        counts_b = sample_clustered_counts(
            n,
            sample_count,
            27400000 + schedule_index * 200000 + rep * 20000,
            jitter_num,
            jitter_den,
        )
        row = split_likelihood_details(counts_a, counts_b, p_law)
...
print("\n(5) Accumulated positive rare-order contributions")
...
visible = {
    name: schedule_rows[name]["excess_mean"] > excess_guard or schedule_rows[name]["tv_mean"] > tv_guard
...
check(
    "Exact zero-jitter clustered law has huge direct unlabeled second moment",
...
check(
    "Sublinear/quarter schedule is visible at exact P_8",
    visible["linear_quarter"],
    f"quarter_excess={fmt(schedule_rows['linear_quarter']['excess_mean'], 12)}",
)
check(
    "Linear half/one/two schedules stay inside the hostile split-null guard",
    not visible["linear_half"] and not visible["linear_one"] and not visible["linear_two"],
    ", ".join(f"{name}:{'V' if visible[name] else 'I'}" for name in ("linear_half", "linear_one", "linear_two")),
)
check(
    "No stable large rare-order contribution survives above the half scale",
    top_linear_contribution < mp.mpf("0.25"),
    f"top_accumulated_positive={fmt(top_linear_contribution, 12)}",
)

print("\n=== Consequence ===")
print("At exact P_8 the direct unlabeled second moment sees sharp clustering")
print("and sees the quarter schedule.  The half/one/two linear schedules stay")
print("inside the hostile split-null guard, and no stable rare-order class")
print("survives the accumulated-top audit.  The honest next theorem is a")
print("bounded-width linear-window contiguity/washout statement, or else a")
print("new exact-denominator computation at larger N that exhibits a residue.")
